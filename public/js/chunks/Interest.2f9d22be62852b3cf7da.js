...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    addInterest: function addInterest(interest) {
      var check = this.picked.includes(interest);
      console.log(check);

      if (check == false) {
        this.picked.push(interest);
      } else {
        this.finalPicks = this.picked.filter(function (pick) {
    ...
    saveInterests: function saveInterests() {
      var _this = this;
      ...
      axios.post('/save-user-interest', {
        ...
        if (response.status == 200) {
          console.log(response.data.suggested_);
          _this.$root.loadInterestModal = false;
        } else {
