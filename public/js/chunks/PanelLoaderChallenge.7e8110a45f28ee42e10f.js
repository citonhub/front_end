  ...
  methods: {
      ...
      if (this.$root.selectedChallenge.judges) {
        if (this.$root.selectedChallenge.judges == 'everyone') {
          ...
          var UserJudge = this.$root.selectedChallenge.selected_judges.filter(function (eachJudge) {
            return eachJudge.tempId == _this2.$root.user_temp_id;
          });
          ...
          if (InputFound != null) {
            if (InputFound.length > 0) {
              ...
              InputFound.forEach(function (input) {
                ...
                _this3.$root.projectInputData.push(inputData);
              });
      ...
      axios.post('/run-code-on-sandbox-project', {
        ...
        if (response.status == 200) {
          ...
          if (response.data[0][0].status.description == 'Accepted') {
            ...
            _this5.pageContent = response.data[0].stdout + '\n Error: \n' + response.data[0].stderr + ' \n' + response.data[0].compile_output + '\n' + response.data[0].error;
          }
        }
      })["catch"](function (error) {
        _this5.pageContent = 'Oops! This participant hasn\'t written any codes yet.';
      });
    },
    ...
    fetchParticipants: function fetchParticipants() {
      ...
      axios.get('/fetch-challenge-participants/' + this.$route.params.challenge_id).then(function (response) {
        if (response.status == 200) {
          _this6.participants = response.data.participants;
          _this6.$root.selectedChallenge = response.data.challenge[0];

          _this6.participants.sort(function (a, b) {
            return b.stars - a.stars;
          });

          _this6.votes = response.data.votes;
          _this6.selectedParticipantId = ''; // show first result
        }
      })["catch"](function (error) {
        _this6.showAlert('Oops!', 'Unable to fetch participants', 'error');
      });
    },
    SaveVotes: function SaveVotes() {
      ...
      setTimeout(function () {
        if (_this7.votes.length == 0) {
          return;
        }

        _this7.saveLoading = true;
        axios.post('/save-challenge-votes', {
          ...
          if (response.status == 200) {
            ...
            _this7.showAlert('Nice!', 'Your votes have been save', 'success');
          }
        })["catch"](function (error) {
          _this7.saveLoading = false;

          _this7.showAlert('Oops!', 'Unable to save votes', 'error');
        });
      }, 1000);
    }
  }
...
var render = function() {
  ...
  return _c(
    ...
    [
        ...
        [
          _c("div", { staticClass: "row" }, [
            ...
            _c(
              ...
              [
                _vm.votes.length != 0 && _vm.checkIfCanVote()
                  ? [
      ...
      _vm._v(" "),
      _vm.selectedParticipantId != "" && _vm.checkIfCanVote()
        ? _c(
            "v-btn",
            {
              staticClass: "d-inline-block ",
