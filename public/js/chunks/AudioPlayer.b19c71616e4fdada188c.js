  ...
  methods: {
    ...
    load: function load() {
      ...
      audioDownload.then(function (result) {
        var audioBlob = new Blob(result.response);
      ...
      console.log(audio);
      return; //Wait for audio to load, then run initSlider() to get audio duration and set the max value of our slider 
      ...
      this.$watch("playbackTime", function () {
        var audio = document.querySelector('#audio' + this.playerId);
        var diff = Math.abs(this.playbackTime - this.$refs.player.currentTime); //Throttle synchronization to prevent infinite loop between playback listener and this watcher
    ...
    playbackListener: function playbackListener(e) {
      ...
      this.playbackTime = audio.currentTime; //console.log("update: " + audio.currentTime);
