...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    leaveSpace: function leaveSpace() {
      ...
      storedChat.then(function (result) {
        if (result != null) {
          ...
          _this5.$root.LocalStore('user_chat_list' + _this5.$root.username, finalResult, false, 'leave_space');
        }
      });
    },
    ...
    checkDiary: function checkDiary() {
      ...
      axios.get('/check-diary/' + botId).then(function (response) {
        if (response.status == 200) {
          ...
          var storedChat = _this8.$root.getLocalStore('user_chat_list' + _this8.$root.username);
          ...
          storedChat.then(function (result) {
            if (result != null) {
              ...
              if (userSpace.length > 0) {} else {
                ...
                _this8.$root.LocalStore('user_chat_list' + _this8.$root.username, finalResult);
        ...
        if (this.$root.ChatList.length == 0) {
          ...
          storedChat.then(function (result) {
            if (result != null) {
              ...
              _this11.$root.ChatList.map(function (chat) {
                chat.subspace_messages = 0;
              });

              _this11.$root.sortChatList();

              _this11.SetUnread(); // get all new messages for sub space


              _this11.checkChannelSubSpace();
              ...
              axios.get('/fetch-chat-list/' + _this11.$root.userDeviceId).then(function (response) {
                if (response.status == 200) {
                  ...
                  _this11.$root.ChatList.map(function (chat) {
                    chat.subspace_messages = 0;
                  }); // get all new messages for sub space


                  _this11.checkChannelSubSpace();

                  _this11.$root.LocalStore('user_chat_list' + _this11.$root.username, response.data, false, 'sort_chat');
    ...
    checkChannelSubSpace: function checkChannelSubSpace() {
      var _this12 = this;

      this.$root.ChatList.map(function (space) {
        var storedSubChat = _this12.$root.getLocalStore('sub_channels_' + space.space_id + _this12.$root.username);

        storedSubChat.then(function (result) {
          if (result != null) {
            var finalResult = JSON.parse(result);
            var subspace = finalResult;

            if (subspace.sub_channels) {
              subspace = subspace.sub_channels;
            }

            space.subspace_messages = _this12.checkForUnreadSubSpace(subspace);
            console.log(_this12.checkForUnreadSubSpace(subspace));
          }
        });
      });
    },
      ...
      if (this.$root.checkauthroot == 'auth') {
        ...
        storedMsg.then(function (result) {
          if (result != null) {
            ...
            _this19.generateUnreadMessage(); // check unreadMessages in subspaces
    ...
    checkForUnreadSubSpace: function checkForUnreadSubSpace(subSpaces) {
      ...
      var fullCount = 0;
      subSpaces.forEach(function (space) {
        var unreadStoredMsg = _this21.$root.getLocalStore('unread_messages_' + space.space_id + _this21.$root.username);

        unreadStoredMsg.then(function (result) {
          if (result != null) {
            var finalResultUnread = JSON.parse(result);
            console.log(finalResultUnread.length);
            fullCount += finalResultUnread.length;
          }
    ...
    fetchSpaceInfo: function fetchSpaceInfo() {
      ...
      axios.get('/fetch-space-info-' + this.$root.selectedSpace.space_id).then(function (response) {
        if (response.status == 200) {
          ...
          var storedChat = _this24.$root.getLocalStore('user_chat_list' + _this24.$root.username);
          ...
          storedChat.then(function (result) {
            if (result != null) {
              ...
              _this24.$root.LocalStore('user_chat_list' + _this24.$root.username, finalResult);
            }
          });
        }
      })["catch"](function (error) {});
                                    ...
                                    scopedSlots: _vm._u(
                                      [
                                        ...
                                        },
                                        {
                                          ...
                                          fn: function() {
                                            return [
                                              _vm.that.$root.channelChats
                                                .length == 0
                                                ? _c(
                                                    "div",
                                                    {
                                                      staticClass:
                                                        "col-12 mt-2 text-center d-flex flex-column"
                                                    },
                                                    [
                                                      _c(
                                                        "div",
                                  ...
                                  _vm.that.$root.selectedSpace.type == "Team"
                                    ? [
                                        _c("invitation", {
                                          attrs: {
                                            infoText:
                                              "A brand new channel and the beginning of your journey to help others grow 🚀",
                                            extraInfo:
                                              "Now, invite others to your channel",
                                            fromChat: true,
                                            alertComponent: this
                                          }
                                        })
                        ...
                        [
                          ...
                          _c("invitation", {
                            attrs: {
                              infoText:
                                "A brand new channel and the beginning of your journey to help others grow 🚀",
                              extraInfo: "Now, invite others to your channel",
                              fromChat: true,
                              alertComponent: this
                            }
                          })
                            ...
                            scopedSlots: _vm._u(
                              [
                                ...
                                },
                                {
                                  ...
                                  fn: function() {
                                    return [
                                      _vm.that.$root.channelChats.length == 0
                                        ? _c(
                                            "div",
                                            {
                                              staticClass:
