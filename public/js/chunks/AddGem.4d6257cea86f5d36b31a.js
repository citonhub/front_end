...
/* harmony default export */ __webpack_exports__["default"] = ({
  data: function data() {
    ...
    return _ref = {
      ...
      "id": 46,
      "name": 'TYPESCRIPT'
    }]), _defineProperty(_ref, "CodeFilename", 'index'), _ref;
  },
  components: {
  ...
  methods: {
    ...
    saveContentOrder: function saveContentOrder() {
      ...
      this.$root.noteContent.pages[this.selectedContentId].contents.forEach(function (message) {
        ContentArray.push(note.message_id);
      });
      axios.post('/save-content-order', {
        ...
        _this2.$root.diaryBoardComponent.showAlert('Oops!', 'Unable to add note,please try again', 'error');
      });
    },
    ...
    filehandler: function filehandler(e) {
      ...
      if (fileSize <= 400000000) {
        fr.addEventListener('load', function () {
          _this5.fileUrl = fr.result; // send message
    ...
    vidoehandler: function vidoehandler(e) {
      ...
      if (fileSize <= 100000000) {
        fr.addEventListener('load', function () {
          _this7.videoUrl = fr.result; // send message
    ...
    sendMessageData: function sendMessageData(formData) {
      ...
      axios.post('/create-response-content', formData, {
        ...
        onUploadProgress: function onUploadProgress(progressEvent) {
          _this8.progressvalue = parseInt(Math.round(progressEvent.loaded / progressEvent.total * 100));
        }
      }).then(function (response) {
        if (response.status == 200) {
          var message = response.data.message;

          _this8.$root.noteContent.pages[_this8.selectedContentId].contents.push(message);

          _this8.clearData();

          _this8.loadingSendMsg = false;
          _this8.progressvalue = 0;
          _this8.$root.AddModalIsUp = false;
        }
      })["catch"](function (error) {
