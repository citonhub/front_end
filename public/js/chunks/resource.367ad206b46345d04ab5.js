...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: (_methods = {
    importDevtoArticle: function importDevtoArticle() {
      ...
      axios.get('/connect-devto/' + this.$root.selectedSpace.space_id).then(function (response) {
    ...
    deleteResource: function deleteResource(resource) {
      ...
      axios.post('/delete-resources', {
        ...
        if (response.status == 200) {
          _this4.$root.chatComponent.showAlert('Deleted!', 'Resource has been deleted', 'success');

          _this4.$root.LocalStore('channel_resource_' + _this4.$root.selectedSpace.space_id + _this4.$root.username, _this4.response);

          _this4.saveResourceOrder(false);
        }
      })["catch"](function (error) {
        _this4.$root.chatComponent.showAlert('Oops!', 'Unable to delete resource,please try again', 'error');
      });
    },
    ...
    storedResouces.then(function (result) {
      if (result != null) {
        ...
        _this5.loadingResources = false;

        _this5.checkForNewResource();
      } else {
        axios.get("/fetch-resource/".concat(_this5.$root.selectedSpace.space_id, "/resource")).then(function (response) {
          if (response.status == 200) {
            _this5.$root.LocalStore('channel_resource_' + _this5.$root.selectedSpace.space_id + _this5.$root.username, response.data.resources);

            var _finalResult = response.data.resources;
            _this5.has_devto_resource = response.data.has_devto_resource;
    ...
    axios.post('/save-resources-order', {
      ...
      if (response.status == 200) {
        if (showAlert) {
          _this6.$root.chatComponent.showAlert('Saved!', 'Your changes have been saved', 'success');
        }

        _this6.$root.LocalStore('channel_resource_' + _this6.$root.selectedSpace.space_id + _this6.$root.username, _this6.resources);
      }
    })["catch"](function (error) {
      _this6.$root.chatComponent.showAlert('Oops!', 'Unable to save changes,please try again', 'error');
    });
    ...
    axios.post('/create-resource', {
      ...
      if (response.status == 200) {
        ...
        var storedResouces = _this8.$root.getLocalStore('channel_resource_' + _this8.$root.selectedSpace.space_id + _this8.$root.username);
        ...
        storedResouces.then(function (result) {
          if (result != null) {
            ...
            _this8.$root.LocalStore('channel_resource_' + _this8.$root.selectedSpace.space_id + _this8.$root.username, finalResult);
          } else {}
        });

        _this8.resources.unshift(response.data.resource);
              ...
              ? [
                  ...
                  _vm.showAddArticle
                    ? _c(
                        "div",
                        {
                          staticClass: "col-12 px-0  d-flex flex-row",
                          staticStyle: { "align-items": "center" }
                        },
                        [
                          _c("input", {
