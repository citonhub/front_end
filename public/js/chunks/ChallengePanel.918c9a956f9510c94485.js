  ...
  methods: {
    ...
    joinChallenge: function joinChallenge(project) {
      ...
      axios.post('/join-challenge', {
        ...
        if (response.status == 200) {
          _this3.loadingJoinAction = false;
          _this3.$root.selectedChallenge.user_participating = true;
          _this3.$root.selectedChallenge.participant_data = response.data.participant;

          _this3.$root.LocalStore('challenge_data_' + _this3.$route.params.challenge_id + _this3.$root.username, _this3.$root.selectedChallenge);

          var storedProjects = _this3.$root.getLocalStore('user_projects_' + _this3.$root.username);

          storedProjects.then(function (result) {
            if (result != null) {
              var finalResult = JSON.parse(result);
              _this3.$root.projectList = finalResult;

              _this3.$root.projectList.user_projects.unshift(project);

              _this3.$root.LocalStore('user_projects_' + _this3.$root.username, _this3.$root.projectList);
            }
          });
          _this3.loadingJoinAction = false;
          var projectSlug = _this3.$root.selectedChallenge.participant_data.project_slug;
          _this3.$root.panelFromChallenges = true;
    ...
    createProject: function createProject(lang) {
      ...
      axios.post('/create-project', {
        ...
        if (response.status == 200) {
          _this5.joinChallenge(response.data.project);
        }
      })["catch"](function (error) {
        _this5.loadingJoinAction = false;
      });
    },
    ...
    fetchChallenge: function fetchChallenge() {
      ...
      storedProjectData.then(function (result) {
        if (result != null) {
          var finalResult = JSON.parse(result);
          _this8.$root.selectedChallenge = finalResult;
          _this8.countDownDate = new Date(_this8.$root.selectedChallenge.duel_terminal_time).getTime();

          var status = _this8.checkDuelStatus(_this8.$root.selectedChallenge);
          ...
          _this8.loadingChallenge = false;

          _this8.getChallengeLanguages();

          _this8.updateChallenge();
        } else {
          axios.get('/fetch-challenge-data/' + _this8.$route.params.challenge_id).then(function (response) {
            if (response.status == 200) {
              _this8.$root.LocalStore('challenge_data_' + _this8.$route.params.challenge_id + _this8.$root.username, response.data[0]);

              _this8.$root.selectedChallenge = response.data[0];
              _this8.countDownDate = new Date(_this8.$root.selectedChallenge.duel_terminal_time).getTime();
  ...
  return _c("div", [
                ...
                _c(
                  ...
                  [
                    _c(
                      ...
                      {
                        staticClass:
                          "col-lg-6 offset-lg-3 py-1  col-md-8 offset-md-2 scroller",
                        staticStyle: {
                          "max-height": "400px",
                          "overflow-y": "auto"
                        }
    ...
    this.$root.showLanguageOption
      ? _c(
          "div",
          {
            staticClass: "py-0 px-0 d-md-none d-block",
            staticStyle: {
              position: "fixed",
              left: "0%",
              width: "100%",
