...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    saveResourceContentOrder: function saveResourceContentOrder() {
      var _this = this;
      ...
      axios.post('/save-resources-content-order', {
        ...
        if (response.status == 200) {
          _this.$root.LocalStore('channel_resource_content_' + _this.$root.selectedResource.resource_id + _this.$root.username, _this.contents);

          if (showAlert) {
            _this.$root.chatComponent.showAlert('Saved!', 'Your changes have been saved', 'success');
          }

          _this.$root.loadingResourceContent = false;
        }
      })["catch"](function (error) {
        _this.$root.chatComponent.showAlert('Oops!', 'Unable to save changes,please try again', 'error');
    ...
    showOptions: function showOptions(content) {
      ...
      if (this.selected_id == content.id) {
        axios.post('/delete-resource-content', {
          ...
          if (response.status == 200) {
            var newContents = _this3.contents.filter(function (content) {
              return content.id !== _this3.selected_id;
            });

            _this3.contents = newContents;

            _this3.$root.LocalStore('channel_resource_content_' + _this3.$root.selectedResource.resource_id + _this3.$root.username, _this3.contents);
          }
        })["catch"](function (error) {
          _this3.selected_id = '';
    ...
    handleResource: function handleResource(content, index) {
      ...
      if (this.fromStandalone && this.$root.isLogged) {
      ...
      if (content.type == 'youtube_video') {
        if (_typeof(content.content.id) === 'object') {
          if (!this.$root.isLogged) {
            this.$root.checkIfUserIsLoggedIn();
            this.$root.showResourceView = false;
            return;
          ...
          this.$router.push({
            path: '/channels/' + this.$root.selectedSpace.space_id + '/resource_content_new/template'
          });
        } else {
    ...
    checkIfSelected: function checkIfSelected(content) {
      var thisContent = this.$root.selectedItems.filter(function (eachContent) {
    ...
    addToSelected: function addToSelected(content) {
      if (this.checkIfSelected(content)) {
        var remainingContent = this.$root.selectedItems.filter(function (eachContent) {
          return content.content.id != eachContent.id;
        });
                          ...
                          _c(
                            ...
                            [
                              ...
                              _c(
                                ...
                                {
                                  class: _vm.isFree
                                    ? "col-7 d-flex flex-column pt-1 my-auto"
                                    : "col-5 d-flex flex-column pt-1 my-auto",
                                  staticStyle: {
                                    "justify-content": "center",
                                    width: "100%"
                                  },
                              ...
                              _c(
                                ...
                                [
                                  _vm.show_add_icon
                                    ...
                                    : _vm._e(),
                                  _vm._v(" "),
                                  _vm.checkIfisOwner() && !_vm.show_add_icon
                                    ? [
                                        _c(
                                          "v-btn",
                                          {
