    ...
    checkForNewDiary: function checkForNewDiary() {
      ...
      axios.get('/get-diary-bank').then(function (response) {
        if (response.status == 200) {
          _this.$root.LocalStore('user_diary_bank_' + _this.$root.username, response.data.user_diary);

          _this.$root.diaryBankList = response.data.user_diary;
        }
    ...
    fetchDiaryBank: function fetchDiaryBank() {
      ...
      storedDiary.then(function (result) {
        if (result != null) {
          ...
          axios.get('/get-diary-bank').then(function (response) {
            if (response.status == 200) {
              _this2.$root.LocalStore('user_diary_bank_' + _this2.$root.username, response.data.user_diary);

              _this2.$root.diaryBankList = response.data.user_diary;
              _this2.loadingDiary = false;
            }
          })["catch"](function (error) {
            _this2.loadingDiary = false;
  ...
  return _c(
    ...
    [
      ...
      _c(
        ...
        [
          _c("span", { staticStyle: { "font-size": "12px", color: "white" } }, [
            _vm._v("my diaries")
          ])
        ]
      ),
            ...
            this.$root.TopBarComponentBoard
              ? [
                  this.$root.TopBarComponentBoard.searchContent.length > 0
                    ? _vm._l(this.$root.diaryBankSearchList, function(diary) {
                        return _c(
                          "div",
                          {
                            key: diary.id + "search",
