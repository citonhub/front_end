  ...
  methods: {
    ...
    saveContentOrder: function saveContentOrder() {
      ...
      axios.post('/save-content-order', {
        ...
        if (response.status == 200) {
          if (showAlert) {
            _this5.$root.diaryBoardComponent.showAlert('Saved!', 'Your changes have been saved', 'success');
          }

          _this5.$root.LocalStore('user_diary_data_' + _this5.$route.params.diary_id + _this5.$root.username, _this5.$root.selectedDiary);
        }
      })["catch"](function (error) {
        _this5.$root.diaryBoardComponent.showAlert('Oops!', 'Unable to save changes,please try again', 'error');
      });
    },
    ...
    runCodeOnSandbox: function runCodeOnSandbox() {
      ...
      axios.post('/run-code-on-sandbox', {
        ...
        _this6.ResultCode = 'An issue occured,unable to run on sandbox...' + error;
      });
    },
    addNewContent: function addNewContent() {
      ...
      axios.get('/add-new-page/' + this.$root.noteContent.note.tag_unique_id + '/' + this.$route.params.diary_id).then(function (response) {
        if (response.status == 200) {
          _this7.$root.noteContent.pages.push(response.data);

          _this7.loadingAddPage = false;
        }
      })["catch"](function (error) {
        _this7.$root.diaryBoardComponent.showAlert('Oops!', 'Unable to add page,please try again', 'error');
    ...
    filehandler: function filehandler(e) {
      ...
      if (fileSize <= 400000000) {
        fr.addEventListener('load', function () {
          _this9.fileUrl = fr.result; // send message
    ...
    vidoehandler: function vidoehandler(e) {
      ...
      if (fileSize <= 100000000) {
        fr.addEventListener('load', function () {
          _this11.videoUrl = fr.result; // send message
    ...
    sendMessageData: function sendMessageData(formData) {
      ...
      axios.post('/create-response-content', formData, {
        ...
        onUploadProgress: function onUploadProgress(progressEvent) {
          _this12.progressvalue = parseInt(Math.round(progressEvent.loaded / progressEvent.total * 100));
        }
      }).then(function (response) {
        if (response.status == 200) {
          var message = response.data.message;

          _this12.$root.noteContent.pages[_this12.selectedContentId].contents.push(message);

          _this12.saveContentOrder(false);

          _this12.clearData();

          _this12.loadingSendMsg = false;
          _this12.progressvalue = 0;
          _this12.$root.AddModalIsUp = false;
        }
      })["catch"](function (error) {
        _this12.showAlert('Oops!', 'Something went wrong, please try again', 'error');
