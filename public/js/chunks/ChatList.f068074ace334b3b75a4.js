          ...
          storedChat.then(function (result) {
            if (result != null) {
              ...
              fullList.map(function (space) {
                space.unread = _this.getUnread(space.space_id);
              });
              _this.$root.ChatList = fullList;

              _this.$root.sortChatList(); // get all new messages


              _this.$root.updateSpaceMessages();
            } else {
              axios.get('/fetch-chat-list/' + _this.$root.userDeviceId).then(function (response) {
                if (response.status == 200) {
                  var responseList = response.data;
                  _this.$root.ChatList = responseList.channels.concat(responseList.direct_messages, responseList.pet_spaces);

                  _this.$root.LocalStore('user_chat_list' + _this.$root.username, response.data);
