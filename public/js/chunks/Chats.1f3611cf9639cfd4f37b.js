  ...
  methods: {
    ...
    showSideBarHandler: function showSideBarHandler() {
      ...
      if (!this.$root.authProfile.user_onboarded) {
        axios.post('/save-user-onboarded-status').then(function (response) {
          if (response.status == 200) {
            _this6.$root.authProfile.user_onboarded = true;
          }
        })["catch"](function (error) {});
      }
    },
    checkDiary: function checkDiary() {
      ...
      axios.get('/check-diary/' + botId).then(function (response) {
        if (response.status == 200) {
          ...
          var storedChat = _this7.$root.getLocalStore('user_chat_list' + _this7.$root.username);
          ...
          storedChat.then(function (result) {
            if (result != null) {
              ...
              if (userSpace.length > 0) {} else {
                ...
                _this7.$root.LocalStore('user_chat_list' + _this7.$root.username, finalResult);

                var fullList = finalResult.channels.concat(finalResult.direct_messages, finalResult.pet_spaces);
                _this7.$root.ChatList = fullList;

                _this7.$root.sortChatList();
              }

              _this7.openChat(response.data.space.space_id, true);

              _this7.$root.showDiarySettings = false;
            }
          });
          _this7.$root.showDiarySettings = false;
        }
      })["catch"](function (error) {
        _this7.showAlert('Oops!', 'Something went wrong, please try again', 'error');
    ...
    searchChatList: function searchChatList() {
      ...
      var chatListResult = this.$root.ChatList.filter(function (chat) {
        ...
        return nameValue.includes(_this8.searchValue.toLowerCase());
      });
      ...
      if (this.$root.checkauthroot == 'auth') {
        if (this.$root.ChatList.length == 0) {
          ...
          storedChat.then(function (result) {
            if (result != null) {
              var finalResult = JSON.parse(result);
              _this9.$root.baseChatList = finalResult;
              _this9.$root.channelChats = finalResult.channels;
              var fullList = finalResult.channels.concat(finalResult.direct_messages, finalResult.pet_spaces);
              _this9.$root.ChatList = fullList;

              _this9.$root.sortChatList();

              _this9.SetUnread(); // get all new messages


              _this9.$root.updateSpaceMessages();

              _this9.$root.loadingChatList = false;
            } else {
              axios.get('/fetch-chat-list/' + _this9.$root.userDeviceId).then(function (response) {
                if (response.status == 200) {
                  var responseList = response.data;
                  _this9.$root.baseChatList = response.data;
                  _this9.$root.channelChats = responseList.channels;
                  _this9.$root.ChatList = responseList.channels.concat(responseList.direct_messages, responseList.pet_spaces);

                  _this9.$root.LocalStore('user_chat_list' + _this9.$root.username, response.data, false, 'sort_chat');

                  _this9.$root.loadingChatList = false;
                }
              })["catch"](function (error) {
                _this9.showAlert('Oops!', 'Unable to fetch channels', 'error');

                _this9.loadingChatList = false;
                _this9.loadingIsError = true;
              });
            }
          });
        } else {
    ...
    SetUnread: function SetUnread() {
      ...
      this.$root.ChatList.map(function (space) {
        var unreadStoredMsg = _this10.$root.getLocalStore('unread_messages_' + space.space_id + _this10.$root.username);
    ...
    checkIfisOwner: function checkIfisOwner() {
      ...
      var userMemberData = this.$root.selectedSpaceMembers.filter(function (members) {
        return members.user_id == _this11.$root.user_temp_id;
      });
        ...
        if (result != null) {
          ...
          if (finalResult.length != 0) {
            for (var index = 0; index < finalResult.length; index++) {
              _this12.$root.sendTextMessage(finalResult[index]);
            }

            _this12.$root.LocalStore('unsent_messages_' + _this12.$route.params.spaceId + _this12.$root.username, []);
          }
        } else {}
      });
    },
    makeUserMaster: function makeUserMaster(member) {
      ...
      axios.post('/make-user-master', {
        ...
        if (response.status == 200) {
          _this13.$root.adminMembers.forEach(function (member) {
            member.master_user = false;
          });

          _this13.$root.adminMembers.map(function (member) {
            if (member.memberId == response.data) {
              member.master_user = true;
            }
          });

          _this13.$root.selectedSpaceMembers.forEach(function (member) {
            member.master_user = false;
          });

          _this13.$root.selectedSpaceMembers.map(function (member) {
            if (member.memberId == response.data) {
              member.master_user = true;
            }
    ...
    autoMakeuserMaster: function autoMakeuserMaster() {
      ...
      if (this.checkIfisOwner()) {
        ...
        var userMember = this.$root.selectedSpaceMembers.filter(function (member) {
          return member.user_id == _this14.$root.user_temp_id;
        });
      ...
      axios.post('/check-for-unread-messages-clean', {
        ...
        if (response.status == 200) {
          if (response.data[0].length > 0) {
            var messageData = {
              space_id: _this15.$root.selectedSpace.space_id,
              new_messages: response.data[0]
            };

            _this15.$root.handleSpaceData([messageData]);
          }
        }
      })["catch"](function (error) {});
    },
    ...
    fetchMessages: function fetchMessages(spaceId) {
      ...
      if (this.$root.checkauthroot == 'auth') {
        ...
        storedMsg.then(function (result) {
          if (result != null) {
            var finalResult = JSON.parse(result);
            _this16.$root.spaceFullData = finalResult;

            var returnedData = _this16.handleResults(finalResult.messages);

            _this16.$root.Messages = returnedData;
            _this16.$root.selectedSpace = [];
            _this16.$root.selectedSpace = finalResult.space;

            if (finalResult.space.type == 'SubSpace') {
              var generalSpace = finalResult.space.gen_space;
              _this16.$root.selectedGenSpaceType = finalResult.space.gen_space.type;
              ...
              _this16.$root.selectedSubSpaceName = finalResult.space.name;
              _this16.$root.selectedSubSpaceType = subSpaceData.type;
              generalSpace.sub_spaces = finalResult.space.sub_spaces;
              _this16.$root.selectedSpace = generalSpace;
            }

            _this16.$root.TrackLastSubSpace.push(_this16.$root.selectedSpace.general_spaceId, _this16.$route.params.spaceId);

            _this16.$root.selectedSpaceMembers = finalResult.members; // set default admin user

            _this16.autoMakeuserMaster(); // get the message scroller component and scroll to bottom


            _this16.$root.msgScrollComponent = _this16.$refs;

            _this16.setShareLink(); // check for space in chatlist


            if (_this16.$root.selectedSpace.type == 'Channel' || _this16.$root.selectedSpace.type == 'Team' || _this16.$root.selectedSpace.type == 'Direct') {
              _this16.$root.addSpaceToChatList(_this16.$root.selectedSpace.space_id);
            } // generate unread msg and the mark as read


            _this16.generateUnreadMessage();

            setTimeout(function () {
              if (_this16.$root.selectedSpace.type == 'Channel' || _this16.$root.selectedSpace.type == 'Team') {
                if (_this16.$refs.messageContainer) {
                  _this16.$refs.messageContainer.scrollToBottom();
                }

                if (_this16.$refs.messageContainersmall) {
                  _this16.$refs.messageContainersmall.scrollToBottom();
                }
              } else {
                if (_this16.$refs.messageContainer) {
                  _this16.$refs.messageContainer.scrollToBottom();
            ...
            _this16.messageIsDone = true; // check for unsent messages and resend

            var unsentStoredMsg = _this16.$root.getLocalStore('unsent_messages_' + _this16.$route.params.spaceId + _this16.$root.username);

            unsentStoredMsg.then(function (result) {
              _this16.resendMessages();
            });

            _this16.fetchSpaceInfo();
          } else {
            axios.get('/fetch-space-messages-' + _this16.$route.params.spaceId).then(function (response) {
              if (response.status == 200) {
                if (response.data.space.space_id == _this16.$route.params.spaceId) {
                  _this16.$root.spaceFullData = response.data;

                  _this16.$root.LocalStore('full_' + _this16.$route.params.spaceId + _this16.$root.username, response.data);

                  _this16.$root.LocalStore('unread_messages_' + _this16.$route.params.spaceId + _this16.$root.username, []);

                  var _returnedData = _this16.handleResults(response.data.messages);

                  _this16.$root.Messages = _returnedData;
                  _this16.$root.selectedSpace = response.data.space;

                  if (response.data.space.type == 'SubSpace') {
                    var _generalSpace = response.data.space.gen_space;
                    _this16.$root.selectedGenSpaceType = response.data.space.gen_space.type;
                    ...
                    _this16.$root.selectedSubSpaceName = response.data.space.name;
                    _this16.$root.selectedSubSpaceType = _subSpaceData.type;
                    _generalSpace.sub_spaces = response.data.space.sub_spaces;
                    _this16.$root.selectedSpace = _generalSpace;
                  }

                  if (response.data.space.type == 'Bot') {
                    _this16.$root.botSuggestionArray = response.data.patterns;

                    _this16.$root.LocalStore('bot_latest_suggestions' + _this16.$root.selectedSpace.space_id + _this16.$root.username, response.data.patterns);
                  }

                  _this16.$root.TrackLastSubSpace.push(_this16.$root.selectedSpace.general_spaceId, _this16.$route.params.spaceId);

                  _this16.$root.selectedSpaceMembers = response.data.members;
                  _this16.$root.msgScrollComponent = _this16.$refs;
                  ...
                  _this16.messageIsDone = true;
                }
              }
            })["catch"](function (error) {
              _this16.errorLoadingMessage = true;
            });
          }

          if (_this16.$root.selectedSpace.type == 'Bot') {
            _this16.checkForNewNotes();
    ...
    checkForUnreadMessagesDisconnected: function checkForUnreadMessagesDisconnected() {
      ...
      if (this.chatIsOpen) {
        ...
        unreadStoredMsg.then(function (result) {
          ...
          if (_this17.$root.sendingMessage == false) {
            _this17.periodicUpdate(finalResultUnread);
          }
        });
      }
    },
    checkForNewNotes: function checkForNewNotes() {
      ...
      axios.get('/fetch-diary-notes-' + this.$root.selectedSpace.bot_data.bot_id).then(function (response) {
        if (response.status == 200) {
          _this18.$root.LocalStore('diary_notes_' + _this18.$root.selectedSpace.space_id + _this18.$root.username, response.data);

          var finalResult = response.data.diary_notes;
          _this18.$root.diaryNotes = finalResult;
        }
      })["catch"](function (error) {});
    },
    fetchSpaceInfo: function fetchSpaceInfo() {
      ...
      axios.get('/fetch-space-info-' + this.$root.selectedSpace.space_id).then(function (response) {
        if (response.status == 200) {
          var storedMsg = _this19.$root.getLocalStore('full_' + _this19.$root.selectedSpace.space_id + _this19.$root.username);
          ...
          storedMsg.then(function (result) {
            if (result != null) {
              ...
              _this19.$root.LocalStore('full_' + _this19.$root.selectedSpace.space_id + _this19.$root.username, finalResult);
            }
          }); // update chatList

          var storedChat = _this19.$root.getLocalStore('user_chat_list' + _this19.$root.username);
            ...
            if (result != null) {
              ...
              if (response.data.space.type == 'Channel' || response.data.space.type == 'Team') {
                finalResult.channels.map(function (chat) {
                  if (chat.space_id == _this19.$root.selectedSpace.space_id) {
              ...
              var fullList = finalResult.channels.concat(finalResult.direct_messages, finalResult.pet_spaces);
              _this19.$root.ChatList = fullList;

              _this19.$root.sortChatList();

              _this19.$root.LocalStore('user_chat_list' + _this19.$root.username, finalResult);
            }
          });
        }
      })["catch"](function (error) {});
      ...
      if (this.$root.Messages.length == 0) {} else {
        ...
        storedSuggestions.then(function (result) {
          ...
          if (finalResultUnread != null) {
            _this20.$root.botSuggestionArray = finalResultUnread;
          }
        });
      }
    }
