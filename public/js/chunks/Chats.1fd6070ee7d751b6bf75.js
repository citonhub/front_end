  ...
  methods: {
      ...
      if (this.$root.checkauthroot == 'auth') {
        if (this.$root.ChatList.length == 0) {
          ...
          storedChat.then(function (result) {
            if (result != null) {
              var finalResult = JSON.parse(result);
              _this11.$root.baseChatList = finalResult;
              _this11.$root.channelChats = finalResult.channels;
              var FinalMessages = finalResult.direct_messages.filter(function (chat) {
                return chat.last_message.length > 0;
              });
              var fullList = finalResult.channels.concat(FinalMessages, finalResult.pet_spaces);
              _this11.$root.ChatList = fullList;

              _this11.$root.sortChatList();

              _this11.SetUnread();

              _this11.$root.updateSpaceMessages();

              _this11.$root.loadingChatList = false; //  this.checkUserLevel();
            } else {
              _this11.showReloader = false;
              axios.get('/fetch-chat-list/' + _this11.$root.userDeviceId).then(function (response) {
                if (response.status == 200) {
                  var responseList = response.data;
                  _this11.$root.baseChatList = response.data;
                  _this11.$root.channelChats = responseList.channels;
                  var FinalLiveMessages = responseList.direct_messages.filter(function (chat) {
                    return chat.last_message.length > 0;
                  });
                  _this11.$root.ChatList = responseList.channels.concat(FinalLiveMessages, responseList.pet_spaces);

                  _this11.$root.LocalStore('user_chat_list_new_' + _this11.$root.username, response.data, false, 'sort_chat');
                ...
                _this11.showAlert('Oops!', 'Unable to fetch channels', 'error');

                _this11.loadingChatList = false;
                _this11.loadingIsError = true;
              });
            }

            _this11.calculateLevel(_this11.$root.authProfile.points);
          });
        } else {
          setTimeout(function () {
            _this11.$root.updateSpaceMessages();
    ...
    SetUnread: function SetUnread() {
      ...
      this.$root.ChatList.map(function (space) {
        var unreadStoredMsg = _this12.$root.getLocalStore('unread_messages_' + space.space_id + _this12.$root.username);
    ...
    checkUserLevel: function checkUserLevel() {
      ...
      StoredPointInfo.then(function (result) {
        if (result != null) {} else {
          var points = _this13.$root.authProfile.points;

          if (points < 100 && _this13.$root.ChatList.length > 0) {
            _this13.$root.showMentorInfo = true;

            _this13.$root.LocalStore('point_info_' + _this13.$root.username, ['shown']);
          }
        }
      });
    },
    ...
    checkIfisOwner: function checkIfisOwner() {
      ...
      var userMemberData = this.$root.selectedSpaceMembers.filter(function (members) {
        return members.user_id == _this14.$root.user_temp_id;
      });
          ...
          (function () {
            ...
            if (finalResult.length != 0) {
              var _loop = function _loop(index) {
                setTimeout(function () {
                  _this15.$root.sendTextMessage(finalResult[index]);
                }, 2000);
              };
              ...
              _this15.$root.LocalStore('unsent_messages_' + _this15.$route.params.spaceId + _this15.$root.username, []);
            }
          })();
        } else {}
      });
    ...
    makeUserMaster: function makeUserMaster(member) {
      ...
      axios.post('/make-user-master', {
        ...
        if (response.status == 200) {
          _this16.$root.adminMembers.forEach(function (member) {
            member.master_user = false;
          });

          _this16.$root.adminMembers.map(function (member) {
            if (member.memberId == response.data) {
              member.master_user = true;
            }
          });

          _this16.$root.selectedSpaceMembers.forEach(function (member) {
            member.master_user = false;
          });

          _this16.$root.selectedSpaceMembers.map(function (member) {
            if (member.memberId == response.data) {
              member.master_user = true;
            }
    ...
    autoMakeuserMaster: function autoMakeuserMaster() {
      ...
      if (this.checkIfisOwner()) {
        ...
        var userMember = this.$root.selectedSpaceMembers.filter(function (member) {
          return member.user_id == _this17.$root.user_temp_id;
        });
      ...
      axios.post('/check-for-unread-messages-clean', {
        ...
        if (response.status == 200) {
          if (response.data[0].length > 0) {
            var messageData = {
              space_id: _this18.$root.selectedSpace.space_id,
              new_messages: response.data[0]
            };

            _this18.$root.handleSpaceData([messageData]);
          }
        }
      })["catch"](function (error) {});
    },
    ...
    fetchMessages: function fetchMessages(spaceId) {
      ...
      if (this.$root.checkauthroot == 'auth') {
        ...
        storedMsg.then(function (result) {
          if (result != null) {
            var finalResult = JSON.parse(result);
            _this19.$root.spaceFullData = finalResult;

            var returnedData = _this19.handleResults(finalResult.messages);

            _this19.$root.Messages = returnedData;
            _this19.$root.selectedSpace = [];
            _this19.$root.selectedSpace = finalResult.space;
            _this19.$root.selectedSpace.subspace_messages = 0;

            if (finalResult.space.type == 'SubSpace') {
              var generalSpace = finalResult.space.gen_space;
              _this19.$root.selectedGenSpaceType = finalResult.space.gen_space.type;
              ...
              _this19.$root.selectedSubSpaceName = finalResult.space.name;
              _this19.$root.selectedSubSpaceType = subSpaceData.type;
              generalSpace.sub_spaces = finalResult.space.sub_spaces;
              _this19.$root.selectedSpace = generalSpace;
            }

            _this19.$root.TrackLastSubSpace.push(_this19.$root.selectedSpace.general_spaceId, _this19.$route.params.spaceId);

            _this19.$root.selectedSpaceMembers = finalResult.members; // set default admin user

            _this19.autoMakeuserMaster(); // get the message scroller component and scroll to bottom


            _this19.$root.msgScrollComponent = _this19.$refs;

            _this19.setShareLink(); // check for space in chatlist


            if (_this19.$root.selectedSpace.type == 'Channel' || _this19.$root.selectedSpace.type == 'Team' || _this19.$root.selectedSpace.type == 'Direct') {
              _this19.$root.addSpaceToChatList(_this19.$root.selectedSpace.space_id);
            } // generate unread msg and the mark as read


            _this19.generateUnreadMessage(); // set up notification 


            _this19.$root.initialPushMangerReg(); // check unreadMessages in subspaces


            if (_this19.$root.selectedSpace.type == 'Channel' || _this19.$root.selectedSpace.type == 'Team' || _this19.$root.selectedSpace.type == 'SubSpace') {
              _this19.handleUnreadUpdate();
            }

            setTimeout(function () {
              if (_this19.$root.selectedSpace.type == 'Channel' || _this19.$root.selectedSpace.type == 'Team') {
                if (_this19.$refs.messageContainer) {
                  _this19.$refs.messageContainer.scrollToBottom();
                }

                if (_this19.$refs.messageContainersmall) {
                  _this19.$refs.messageContainersmall.scrollToBottom();
                }
              } else {
                if (_this19.$refs.messageContainer) {
                  _this19.$refs.messageContainer.scrollToBottom();
            ...
            _this19.messageIsDone = true; // check for unsent messages and resend

            var unsentStoredMsg = _this19.$root.getLocalStore('unsent_messages_' + _this19.$route.params.spaceId + _this19.$root.username);

            unsentStoredMsg.then(function (result) {
              _this19.resendMessages();
            });

            _this19.fetchSpaceInfo();
          } else {
            axios.get('/fetch-space-messages-' + _this19.$route.params.spaceId).then(function (response) {
              if (response.status == 200) {
                if (response.data.space.space_id == _this19.$route.params.spaceId) {
                  _this19.$root.spaceFullData = response.data;

                  _this19.$root.LocalStore('full_space_' + _this19.$route.params.spaceId + _this19.$root.username, response.data);

                  _this19.$root.LocalStore('unread_messages_' + _this19.$route.params.spaceId + _this19.$root.username, []);

                  var _returnedData = _this19.handleResults(response.data.messages);

                  _this19.$root.Messages = _returnedData;
                  _this19.$root.selectedSpace = response.data.space;
                  _this19.$root.selectedSpace.subspace_messages = 0;

                  if (response.data.space.type == 'SubSpace') {
                    var _generalSpace = response.data.space.gen_space;
                    _this19.$root.selectedGenSpaceType = response.data.space.gen_space.type;
                    ...
                    _this19.$root.selectedSubSpaceName = response.data.space.name;
                    _this19.$root.selectedSubSpaceType = _subSpaceData.type;
                    _generalSpace.sub_spaces = response.data.space.sub_spaces;
                    _this19.$root.selectedSpace = _generalSpace;
                  }

                  if (response.data.space.type == 'Bot') {
                    _this19.$root.botSuggestionArray = response.data.patterns;

                    _this19.$root.LocalStore('bot_latest_suggestions' + _this19.$root.selectedSpace.space_id + _this19.$root.username, response.data.patterns);
                  ...
                  _this19.$root.selectedSpaceMembers = response.data.members;
                  _this19.$root.msgScrollComponent = _this19.$refs;
                  ...
                  _this19.$root.initialPushMangerReg();
                  ...
                  _this19.messageIsDone = true;
                }
              }
            })["catch"](function (error) {
              _this19.errorLoadingMessage = true;
            });
          }

          _this19.$root.updateSpaceMessages();

          if (_this19.$root.selectedSpace.type == 'Bot') {
            _this19.checkForNewNotes();

            _this19.botMessager();
          }

          if (_this19.$root.autoOpenResourcePage) {
            _this19.$root.chatComponent.chatInnerSideBar = true;
            _this19.$root.chatComponent.innerSideBarContent = 'resources';
          }
        });
      }
    },
    handleUnreadUpdate: function handleUnreadUpdate() {
      ...
      storedSubChat.then(function (result) {
        if (result != null) {
          ...
          if (_this20.checkIfisOwner()) {
            _this20.$root.subSpaces = finalResult;
          } else {
            _this20.$root.subSpaces = finalResult.filter(function (space) {
              return space.type == 'Public' || space.type == 'Private' && space.is_member == true;
            });
          }

          _this20.checkForUnreadSubSpace(_this20.$root.subSpaces, false);
        }
      });
    },
    checkForUnreadSubSpace: function checkForUnreadSubSpace(subSpaces) {
      ...
      subSpaces.map(function (space) {
        var unreadStoredMsg = _this21.$root.getLocalStore('unread_messages_' + space.space_id + _this21.$root.username);
        ...
        unreadStoredMsg.then(function (result) {
          ...
          if (returnCount) {
            ...
            _this21.$root.selectedSpaceSubMessages = fullCount;
          }
        });
      });
    },
    checkForUnreadMessagesDisconnected: function checkForUnreadMessagesDisconnected() {
      ...
      if (this.chatIsOpen) {
        ...
        unreadStoredMsg.then(function (result) {
          ...
          if (_this22.$root.sendingMessage == false) {
            _this22.periodicUpdate(finalResultUnread);
          }
        });
      }
    },
    checkForNewNotes: function checkForNewNotes() {
      ...
      axios.get('/fetch-diary-notes-' + this.$root.selectedSpace.bot_data.bot_id).then(function (response) {
        if (response.status == 200) {
          _this23.$root.LocalStore('diary_notes_' + _this23.$root.selectedSpace.space_id + _this23.$root.username, response.data);

          var finalResult = response.data.diary_notes;
          _this23.$root.diaryNotes = finalResult;
        }
      })["catch"](function (error) {});
    },
      ...
      if (this.$root.Messages.length == 0) {} else {
        ...
        storedSuggestions.then(function (result) {
          ...
          if (finalResultUnread != null) {
            _this24.$root.botSuggestionArray = finalResultUnread;
          }
        });
      }
    }
                ...
                [
                  ...
                  _vm.chatIsOpen && _vm.chatInnerConent == "channel_invitation"
                    ? _c(
                        ...
                        {
                          staticClass:
                            "col-12 py-2 pt-4 px-0 text-center d-flex flex-row ",
                          staticStyle: {
