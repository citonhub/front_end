...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    likeComment: function likeComment($id) {
      ...
      axios.post('/like-hub-post-comment', formData).then(function (response) {
        if (response.status == 201) {
          if (response.data == "liked") {
            _this4.fetchComments(_this4.$root.selectedPost.id);
          }
        } else {
      ...
      axios.post('/pin-hub-post', formData).then(function (response) {
        if (response.status == 201) {
          if (response.data == "pinned") {
            _this5.$root.selectedPost.isPinned = 1;
          }
        } else {
    ...
    loadPageContent: function loadPageContent(panelId) {
      ...
      axios.get('/run-panel/' + panelId).then(function (response) {
        if (response.status == 200) {
          _this6.loadingCode = false;
          _this6.pageContent = response.data;
        }
      })["catch"](function (error) {
        _this6.showAlert('Oops!', 'Unable to load page content', 'error');
      });
    },
      ...
      axios.post('/run-code-on-sandbox-project', {
        ...
        if (response.status == 200) {
          ...
          if (response.data[0][0].status.description == 'Accepted') {
            _this7.pageContent = response.data[0][0].stdout;
          } else if (response.data[0][0].status.description == 'In Queue') {
            _this7.pageContent = 'In Queue...';

            _this7.checkResponse(token, response.data[1]);
          } else if (response.data[0][0].status.description == 'Processing') {
            _this7.pageContent = 'Processing...';
        ...
        _this7.pageContent = 'An issue occured,unable to run on sandbox...';
      });
    },
    fetchPost: function fetchPost() {
      ...
      storedPostData.then(function (result) {
        if (result != null) {
          ...
          _this8.$root.selectedPost = finalResult;

          _this8.fetchComments(_this8.$root.selectedPost.id);

          if (!_this8.$root.selectedPost.link) {
            _this8.showPage();
          }

          _this8.loadingPost = false; // this.updatePost();
        } else {
          axios.get('/fetch-post/' + _this8.$route.params.post_id).then(function (response) {
            if (response.status == 200) {
              _this8.$root.LocalStore('post_data_' + _this8.$route.params.post_id + _this8.$root.username, response.data.data);

              _this8.$root.selectedPost = response.data.data;
              console.log(response.data.data);

              _this8.fetchComments(response.data.data.id);

              if (!_this8.$root.selectedPost.link) {
                _this8.showPage();
              }

              _this8.loadingPost = false;
            }
          })["catch"](function (error) {
            _this8.loadingPost = false;
          });
