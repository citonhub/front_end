...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    saveResourceOrder: function saveResourceOrder() {
      ...
      axios.post('/save-resource-order', {
        space_id: this.$root.selectedSpace.space_id,
        resources: ResourcesArray
      }).then(function (response) {
        if (response.status == 200) {
          if (showAlert) {
            _this2.$root.chatComponent.showAlert('Saved!', 'Your changes have been saved', 'success');
          }

          var storedPlaylist = _this2.$root.getLocalStore('channel_playlist_' + _this2.$root.selectedSpace.space_id + _this2.playlists);
        }
    ...
    fetchPlaylists: function fetchPlaylists() {
      ...
      storedPlaylist.then(function (result) {
        if (result != null) {
          ...
          _this3.checkForNewPlaylist();
        } else {
          axios.get("/fetch-resource/".concat(_this3.$root.selectedSpace.space_id, "/playlist")).then(function (response) {
            if (response.status == 200) {
              _this3.$root.LocalStore('channel_playlist_' + _this3.$root.selectedSpace.space_id + _this3.$root.username, response.data.resources);

              var _finalResult = response.data.resources;
              _this3.playlists = _finalResult;
              _this3.loadingPlaylist = false;
            }
          })["catch"](function (error) {
            _this3.loadingPlaylist = false;
    ...
    createPlaylists: function createPlaylists() {
      ...
      axios.post('/create-resource', {
        ...
        if (response.status == 200) {
          ...
          var storedPlaylist = _this5.$root.getLocalStore('channel_playlist_' + _this5.$root.selectedSpace.space_id + _this5.$root.username);
          ...
          storedPlaylist.then(function (result) {
            if (result != null) {
              ...
              _this5.$root.LocalStore('channel_playlist_' + _this5.$root.selectedSpace.space_id + _this5.$root.username, finalResult);
            } else {}
          });

          _this5.playlists.unshift(response.data.resource);
        }
      })["catch"](function (err) {
        console.log(err);
        _this5.saving = false;
