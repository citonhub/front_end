...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    updateDiary: function updateDiary() {
      var _this = this;

      this.loadingUpdate = true;
      axios.get('/update-dairy-' + this.$route.params.diary_id).then(function (response) {
        if (response.status == 200) {
          _this.loadingUpdate = false;
          _this.$root.selectedDiary.updated = true;

          _this.showAlert('Updated!', 'Your changes have been updated', 'success');
        ...
        _this.showAlert('Oops!', 'Unable to update, please try again', 'error');
      });
    },
    saveNote: function saveNote() {
      ...
      axios.post('/update-note-data', {
        ...
        if (response.status == 200) {
          _this2.showAlert('Saved!', 'Please update diary to effect changes', 'success');

          _this2.loadingSaveNote = false;
        }
      })["catch"](function (error) {
        _this2.showAlert('Oops!', 'Unable to update, please try again', 'error');
      ...
      if (type == 'question') {
        izitoast__WEBPACK_IMPORTED_MODULE_0___default.a.question({
          ...
          buttons: [['<button><b>YES</b></button>', function (instance, toast) {
            axios.post('/delete-intent-tag-' + _this3.$root.intentToDelete).then(function (response) {
              if (response.status == 200) {
                var remainingNotes = _this3.$root.selectedDiary.notes.filter(function (note) {
                  return note.note.tag_unique_id != _this3.$root.intentToDelete;
                });

                _this3.$root.selectedDiary.notes = remainingNotes;

                _this3.$root.LocalStore('user_diary_data_' + _this3.$route.params.diary_id + _this3.$root.username, _this3.$root.selectedDiary);

                _this3.$root.intentToDelete = '';

                _this3.$root.diaryBoardComponent.showAlert('Deleted!', 'Note was deleted', 'success');

                _this3.saveNoteOrder(false);
              }
            })["catch"](function (error) {
              _this3.$root.diaryBoardComponent.showAlert('Oops!', 'Unable to delete note', 'error');
            });
            instance.hide({
    ...
    _c(
      ...
      [
        _c(
          ...
          {
            staticClass:
              "col-lg-10 offset-lg-1  mb-0 col-md-10 offset-md-1 px-0 d-md-block d-none py-0 pb-3 px-0 ",
            staticStyle: {
              position: "absolute",
              left: "0%",
              top: "0%",
                ...
                _c(
                  ...
                  [
                    this.$router.currentRoute.path.indexOf("edit-note") >= 0
                      ...
                      : [
                          this.$root.selectedDiary.updated
                            ? [
                                _c(
                                  "v-btn",
                                  {
