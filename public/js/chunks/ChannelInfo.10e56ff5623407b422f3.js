    ...
    leaveSpace: function leaveSpace() {
      ...
      axios.post('/leave-space', {
        ...
        if (response.status == 200) {
          if (_this2.$root.selectedSpace.type == 'Channel' || _this2.$root.selectedSpace.type == 'Team') {
            _this2.$root.removeLocalStorage(_this2.$route.params.spaceId);

            if (_this2.$root.ChatList[2].length != 0) {
              var remainingSpace = _this2.$root.ChatList[2].filter(function (space) {
                return space.space_id != _this2.$route.params.spaceId;
              });

              _this2.$root.ChatList[2] = remainingSpace;

              _this2.$root.LocalStore('ChatList' + _this2.$root.username, _this2.$root.ChatList);
            }
          }
          ...
          storedChat.then(function (result) {
            if (result != null) {
              ...
              if (_this2.$root.selectedSpace.type == 'Channel' || _this2.$root.selectedSpace.type == 'Team') {
                var _remainingSpace = finalResult.channels.filter(function (space) {
                  return space.space_id != _this2.$route.params.spaceId;
                });

                finalResult.channels = _remainingSpace;
              }

              if (_this2.$root.selectedSpace.type == 'Bot') {
                var _remainingSpace2 = finalResult.pet_spaces.filter(function (space) {
                  return space.space_id != _this2.$route.params.spaceId;
                });

                finalResult.channels = _remainingSpace2;
              }
