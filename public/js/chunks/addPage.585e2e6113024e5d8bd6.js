    ...
    postData: function postData() {
      ...
      if (this.$refs.addPost.validate()) {
        ...
        axios.post('/save-hub-post', formData, {
          ...
          if (response.status == 200) {
            //   let currentPost = this.$root.posts;
            //   let newPost =  [response.data.data];
            //    let NewPostsArray = newPost.concat(currentPost);
            //    this.$root.posts = NewPostsArray;
            // this.$root.posts.unshift(response.data.data);
            axios.get('/fetch-posts').then(function (response) {
              if (response.status == 200) {
                _this5.$root.posts = response.data.data;
                _this5.loadingPost = false;
              }
            })["catch"](function (error) {
              _this5.loadingPost = false;

              _this5.showAlert('Oops!', 'Unable to fetch posts,please try again', 'error');
            });
            _this5.loading = false;

            _this5.goBack();

            _this5.$root.authProfile.points += 20;
