...
/* harmony default export */ __webpack_exports__["default"] = ({
  data: function data() {
    ...
    return _ref = {
      ...
      "id": 46,
      "name": 'TYPESCRIPT'
    }]), _defineProperty(_ref, "CodeFilename", 'index'), _defineProperty(_ref, "loadingAddPage", false), _ref;
  },
  components: {
  ...
  methods: {
    ...
    deletePage: function deletePage(page) {
      this.$root.diaryBoardComponent.showAlert('Oops!', 'Unable to save changes,please try again', 'question');
    },
    ...
    runCodeOnSandbox: function runCodeOnSandbox() {
      ...
      axios.post('/run-code-on-sandbox', {
        ...
        if (response.status == 200) {
          ...
          if (response.data[0].status.description == 'Accepted') {
            _this3.ResultCode = response.data[0].stdout;
          } else if (response.data[0].status.description == 'In Queue') {
            _this3.ResultCode = 'In Queue...';

            _this3.checkResponse(token);
          } else if (response.data[0].status.description == 'Processing') {
            _this3.ResultCode = 'Processing...';
        ...
        _this3.ResultCode = 'An issue occured,unable to run on sandbox...' + error;
      });
    },
    addNewContent: function addNewContent() {
      ...
      axios.get('/add-new-page/' + this.$root.noteContent.note.tag_unique_id + '/' + this.$route.params.diary_id).then(function (response) {
        if (response.status == 200) {
          _this4.$root.noteContent.pages.push(response.data);

          _this4.loadingAddPage = false;
        }
      })["catch"](function (error) {
        _this4.$root.diaryBoardComponent.showAlert('Oops!', 'Unable to add page,please try again', 'error');
    ...
    filehandler: function filehandler(e) {
      ...
      if (fileSize <= 400000000) {
        fr.addEventListener('load', function () {
          _this6.fileUrl = fr.result; // send message
    ...
    vidoehandler: function vidoehandler(e) {
      ...
      if (fileSize <= 100000000) {
        fr.addEventListener('load', function () {
          _this8.videoUrl = fr.result; // send message
    ...
    sendMessageData: function sendMessageData(formData) {
      ...
      axios.post('/create-response-content', formData, {
        ...
        onUploadProgress: function onUploadProgress(progressEvent) {
          _this9.progressvalue = parseInt(Math.round(progressEvent.loaded / progressEvent.total * 100));
        }
      }).then(function (response) {
        if (response.status == 200) {
          var message = response.data.message;

          _this9.$root.noteContent.pages[_this9.selectedContentId].contents.push(message);

          _this9.saveContentOrder(false);

          _this9.clearData();

          _this9.loadingSendMsg = false;
          _this9.progressvalue = 0;
          _this9.$root.AddModalIsUp = false;
        }
      })["catch"](function (error) {
        _this9.showAlert('Oops!', 'Something went wrong, please try again', 'error');
