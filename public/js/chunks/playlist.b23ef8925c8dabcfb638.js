  ...
  methods: {
    ...
    deleteResource: function deleteResource(resource) {
      ...
      axios.post('/delete-resources', {
        ...
        if (response.status == 200) {
          _this.$root.chatComponent.showAlert('Deleted!', 'Playlist has been deleted', 'success');
        ...
        _this.$root.chatComponent.showAlert('Oops!', 'Unable to delete playlist,please try again', 'error');
      });
    },
    ...
    saveResourceOrder: function saveResourceOrder() {
      ...
      axios.post('/save-resources-order', {
        ...
        if (response.status == 200) {
          if (showAlert) {
            _this4.$root.chatComponent.showAlert('Saved!', 'Your changes have been saved', 'success');
          }

          _this4.$root.LocalStore('channel_playlist_' + _this4.$root.selectedSpace.space_id + _this4.$root.username, _this4.playlists);
        }
      })["catch"](function (error) {
        _this4.$root.chatComponent.showAlert('Oops!', 'Unable to save changes,please try again', 'error');
      });
    },
    ...
    fetchPlaylists: function fetchPlaylists() {
      ...
      storedPlaylist.then(function (result) {
        if (result != null) {
          ...
          _this5.playlists = finalResult;
          _this5.loadingPlaylist = false;

          _this5.checkForNewPlaylist();
        } else {
          axios.get("/fetch-resource/".concat(_this5.$root.selectedSpace.space_id, "/playlist")).then(function (response) {
            if (response.status == 200) {
              _this5.$root.LocalStore('channel_playlist_' + _this5.$root.selectedSpace.space_id + _this5.$root.username, response.data.resources);

              var _finalResult = response.data.resources;
              _this5.has_youtube_resource = response.data.has_youtube_resource;
              _this5.youtube_connected = response.data.youtube_connected;
              _this5.playlists = _finalResult;
              _this5.loadingPlaylist = false;
            }
          })["catch"](function (error) {
            _this5.loadingPlaylist = false;
    ...
    createPlaylists: function createPlaylists() {
      ...
      axios.post('/create-resource', {
        ...
        if (response.status == 200) {
          ...
          var storedPlaylist = _this7.$root.getLocalStore('channel_playlist_' + _this7.$root.selectedSpace.space_id + _this7.$root.username);
          ...
          storedPlaylist.then(function (result) {
            if (result != null) {
              ...
              _this7.$root.LocalStore('channel_playlist_' + _this7.$root.selectedSpace.space_id + _this7.$root.username, finalResult);
            } else {}
          });

          _this7.playlists.unshift(response.data.resource);
