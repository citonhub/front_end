...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  mounted: function mounted() {
    ...
    if (this.$root.isLogged) {
      ...
      storedMsg.then(function (result) {
        _this.$root.autoOpenResourcePage = false;

        if (result != null) {
          _this.$root.chatComponent.openChat(_this.$route.params.spaceId, true);
        } else {
          _this.$root.fromSupportDirectlink = false;

          _this.$router.push({
            path: '/channels/' + _this.$route.params.spaceId + '/payment'
          });
        }
      });
    } else {
  ...
  methods: {
    ...
    showContent: function showContent(content) {
      this.$router.push({
        path: '/channels/e_resources/content/' + this.$route.params.spaceId
      });
    ...
    fetchResourceContent: function fetchResourceContent() {
      ...
      if (this.selectedResource.youtube_playlist_id) {
        ...
        axios.get('/fetch-resource-content/' + this.selectedResource.resource_id + '/' + this.selectedResource.youtube_playlist_id).then(function (response) {
          if (response.status == 200) {
            ...
            _this2.resourcesContent = finalResult;
            _this2.nextPageToken = response.data.nextPageToken;
            _this2.loadingResourceContent = false;
          }
      ...
      if (this.selectedResource.youtube_playlist_id == null && this.selectedResource.devto_username == null) {
        ...
        axios.get('/fetch-resource-content/' + this.selectedResource.resource_id).then(function (response) {
          if (response.status == 200) {
            _this2.resourcesContent = response.data.contents;
            _this2.loadingResourceContent = false;
          }
        })["catch"](function (error) {
          _this2.loadingResourceContent = false;
    ...
    fetchSpaceInfo: function fetchSpaceInfo() {
      ...
      axios.get('/fetch-space-info-' + this.$route.params.spaceId).then(function (response) {
        if (response.status == 200) {
          ...
          _this3.$root.showResourceView = true;
          _this3.mainContent = _this3.spaceData.name + ' Resources';
    ...
    nextPagehandler: function nextPagehandler(shown) {
      ...
      var playlistID = this.selectedResource.youtube_playlist_id;

      if (this.nextPageToken && shown) {
        this.loadingNextPage = true;
        axios.get('/fetch-resource-content/' + this.selectedResource.resource_id + '/' + playlistID + '/' + this.nextPageToken).then(function (response) {
          if (response.status == 200) {
            var results = [];

            if (_this4.selectedResource.devto_username) {
              ...
              _this4.resourcesContent = finalResult;
            } else {
              ...
              _this4.resourcesContent = _finalResult;
            }
