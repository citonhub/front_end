...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    fetchChallenge: function fetchChallenge() {
      ...
      storedProjectData.then(function (result) {
        if (result != null) {
          ...
          _this2.$root.selectedChallenge = finalResult;
          _this2.countDownDate = new Date(_this2.$root.selectedChallenge.duel_terminal_time).getTime();

          var status = _this2.checkDuelStatus(_this2.$root.selectedChallenge);
          ...
          _this2.loadingChallenge = false; // this.checkForNewChallengeData();
        } else {
          axios.get('/fetch-challenge-data/' + _this2.$route.params.challenge_id).then(function (response) {
            if (response.status == 200) {
              _this2.$root.LocalStore('challenge_data_' + _this2.$route.params.challenge_id + _this2.$root.username, response.data[0]);

              _this2.$root.selectedChallenge = response.data[0];
              _this2.loadingChallenge = false;
            }
    ...
    setPage: function setPage() {
      ...
      if (this.$router.currentRoute.path.indexOf('discussion-sm') >= 0) {
        this.selectedTab = 'discussion';
      }
    },
    ...
    goToPage: function goToPage(page) {
      ...
      if (from == 'small') {
        this.$router.push({
          path: '/board/challenges/panel/' + this.$route.params.challenge_id + '/' + page + '-sm'
        });
      } else {
        this.$router.push({
    ...
    checkDuelStatus: function checkDuelStatus(duel) {
      ...
      var votingDateToMoment = moment(duel.duel_voting_time);
                ...
                _c(
                  "div",
                  {
                    ...
                    on: {
                      click: function($event) {
                        $event.stopPropagation()
                        return _vm.goToPage("discussion", "small")
                      }
                    }
                  },
                  [
