    ...
    checkChannelSubSpace: function checkChannelSubSpace() {
      ...
      this.$root.ChatList.map(function (space) {
        ...
        storedSubChat.then(function (result) {
          if (result != null) {
            var finalResult = JSON.parse(result);
            var subspaces = finalResult.sub_channels;
            subspaces.map(function (subspace) {
              var unreadStoredMsg = _this12.$root.getLocalStore('unread_messages_' + subspace.space_id + _this12.$root.username);
