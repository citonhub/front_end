...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    likePost: function likePost() {
      ...
      axios.get("user-liked-post/".concat(this.id)).then(function (response) {
    ...
    runCodeOnSandbox: function runCodeOnSandbox() {
      ...
      axios.post('/run-code-on-sandbox-project', {
        ...
        _this4.pageContent = 'An issue occured,unable to run on sandbox...';
      });
    },
    fetchPost: function fetchPost() {
      ...
      storedPostData.then(function (result) {
        if (result != null) {
          ...
          _this5.$root.selectedPost = finalResult;

          if (!_this5.$root.selectedPost.link) {
            _this5.showPage();
          }

          _this5.loadingPost = false; // this.updatePost();
        } else {
          axios.get('/fetch-post/' + _this5.$route.params.post_id).then(function (response) {
            if (response.status == 200) {
              _this5.$root.LocalStore('post_data_' + _this5.$route.params.post_id + _this5.$root.username, response.data.data);

              _this5.$root.selectedPost = response.data.data;

              if (!_this5.$root.selectedPost.link) {
                _this5.showPage();
              }

              _this5.loadingPost = false;
            }
          })["catch"](function (error) {
            _this5.loadingPost = false;
          });
      ...
      [
        ...
        _c(
          ...
          [
            ...
            _vm._v(" "),
            this.$root.selectedPost.isLiked
              ? _c("v-btn", { attrs: { icon: "" } }, [
                  _c("i", {
                    class: _vm.liked ? "lar la-heart" : "lar la-heart-outline",
