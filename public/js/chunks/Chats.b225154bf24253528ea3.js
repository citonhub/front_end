...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    fetchMessages: function fetchMessages() {
      ...
      this.errorLoadingMessage = false; // proceed if user is logged in

      if (this.$root.checkauthroot == 'auth') {
        // sub sapce tracker
        this.$root.TrackLastSubSpace = []; // get the space data from local storage

        var storedMsg = this.$root.getLocalStore(this.$route.params.spaceId + this.$root.username);
        storedMsg.then(function (result) {
          if (result != null) {
            ...
            setTimeout(function () {
              if (_this9.$root.selectedSpace.type == 'Bot' || _this9.$root.selectedSpace.type == 'Direct') {
                _this9.$root.msgScrollComponent.messageContainer.scrollToBottom();
              } else {
                _this9.$root.msgScrollComponent.messageContainer.scrollToItem(_this9.$root.Messages.length);
              }
            }, 500);
            ...
            axios.get('/fetch-space-messages-' + _this9.$route.params.spaceId).then(function (response) {
              if (response.status == 200) {
                if (response.data.space.space_id == _this9.$route.params.spaceId) {
                  ...
                  _this9.$root.LocalStore(_this9.$route.params.spaceId + _this9.$root.username, response.data);
          ...
          if (_this9.$root.selectedSpace.general_spaceId != undefined) {
            _this9.getMemberUpdates(_this9.$root.selectedSpace.general_spaceId);
          }
        });
      }
  ...
  getMemberUpdates: function getMemberUpdates(spaceId) {
    ...
    axios.get('/fetch-space-members-' + spaceId).then(function (response) {
      if (response.status == 200) {
        _this10.$root.selectedSpaceMembers = response.data;
      }
    })["catch"](function (error) {});
  },
    ...
    if (this.$root.Messages.length == 0) {} else {
      ...
      storedSuggestions.then(function (result) {
        ...
        if (finalResultUnread != null) {
          _this11.$root.botSuggestionArray = finalResultUnread;
        }
      });
    }
  }
                  ...
                  _vm.chatbarContent == "chat_list"
                    ? [
                        _c(
                          ...
                          {
                            staticClass:
                              "col-12 py-2 px-3 pb-0 mb-0 text-center d-flex flex-row fixed-top",
                            staticStyle: {
                              left: "0%",
                              height: "58px",
                              position: "sticky",
