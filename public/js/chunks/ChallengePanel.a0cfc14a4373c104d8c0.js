  ...
  methods: {
    ...
    fetchChallenge: function fetchChallenge() {
      ...
      storedProjectData.then(function (result) {
        if (result != null) {
          var finalResult = JSON.parse(result);
          _this9.$root.selectedChallenge = finalResult;
          ...
          _this9.loadingChallenge = false;

          _this9.getChallengeLanguages();

          _this9.updateChallenge();
        } else {
          axios.get('/fetch-challenge-data/' + _this9.$route.params.challenge_id).then(function (response) {
            if (response.status == 200) {
