        ...
        if (result != null) {
          ...
          axios.get('/fetch-sub-spaces-' + _this2.$root.selectedSpace.general_spaceId + '-' + _this2.$root.userDeviceId).then(function (response) {
            if (response.status == 200) {
              ...
              _this2.$root.LocalStore('sub_channels_' + _this2.$root.selectedSpace.general_spaceId + _this2.$root.username, _this2.$root.subSpaces);
    ...
    checkForNewSubSpace: function checkForNewSubSpace() {
      ...
      axios.get('/fetch-sub-spaces-' + this.$root.selectedSpace.general_spaceId + '-' + this.$root.userDeviceId).then(function (response) {
        if (response.status == 200) {
          ...
          if (_this3.checkIfisOwner()) {
            _this3.$root.subSpaces = finalResult;
          } else {
            _this3.$root.subSpaces = finalResult.filter(function (space) {
              return space.type == 'Public' || space.type == 'Private' && space.is_member == true;
            });
          }

          _this3.$root.LocalStore('sub_channels_' + _this3.$root.selectedSpace.general_spaceId + _this3.$root.username, _this3.$root.subSpaces);

          _this3.checkForUnread();
