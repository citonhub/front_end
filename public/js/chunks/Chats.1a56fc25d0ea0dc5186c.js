  ...
  methods: {
    ...
    showSideBarHandler: function showSideBarHandler() {
      ...
      if (!this.$root.authProfile.user_onboarded) {
        axios.post('/save-user-onboarded-status').then(function (response) {
          if (response.status == 200) {
            _this5.$root.authProfile.user_onboarded = true;
          }
        })["catch"](function (error) {});
      }
    },
    checkDiary: function checkDiary() {
      ...
      axios.get('/check-diary/' + botId).then(function (response) {
        if (response.status == 200) {
          ...
          var storedChat = _this6.$root.getLocalStore('user_chat_list' + _this6.$root.username);
          ...
          storedChat.then(function (result) {
            if (result != null) {
              ...
              if (userSpace.length > 0) {} else {
                ...
                _this6.$root.LocalStore('user_chat_list' + _this6.$root.username, finalResult);

                var fullList = finalResult.channels.concat(finalResult.direct_messages, finalResult.pet_spaces);
                _this6.$root.ChatList = fullList;

                _this6.$root.sortChatList();
              }

              _this6.openChat(response.data.space.space_id, true);

              _this6.$root.showDiarySettings = false;
            }
          });
          _this6.$root.showDiarySettings = false;
        }
      })["catch"](function (error) {
        _this6.showAlert('Oops!', 'Something went wrong, please try again', 'error');
    ...
    searchChatList: function searchChatList() {
      ...
      var chatListResult = this.$root.ChatList.filter(function (chat) {
        ...
        return nameValue.includes(_this7.searchValue.toLowerCase());
      });
      ...
      if (this.$root.checkauthroot == 'auth') {
        if (this.$root.ChatList.length == 0) {
          ...
          storedChat.then(function (result) {
            if (result != null) {
              var finalResult = JSON.parse(result);
              _this8.$root.baseChatList = finalResult;
              _this8.$root.channelChats = finalResult.channels;
              var fullList = finalResult.channels.concat(finalResult.direct_messages, finalResult.pet_spaces);
              _this8.$root.ChatList = fullList;

              _this8.$root.sortChatList();

              _this8.SetUnread(); // get all new messages


              _this8.$root.updateSpaceMessages();

              _this8.$root.loadingChatList = false;
            } else {
              axios.get('/fetch-chat-list/' + _this8.$root.userDeviceId).then(function (response) {
                if (response.status == 200) {
                  var responseList = response.data;
                  _this8.$root.baseChatList = response.data;
                  _this8.$root.channelChats = responseList.channels;
                  _this8.$root.ChatList = responseList.channels.concat(responseList.direct_messages, responseList.pet_spaces);

                  _this8.$root.LocalStore('user_chat_list' + _this8.$root.username, response.data, false, 'sort_chat');

                  _this8.$root.loadingChatList = false;
                }
              })["catch"](function (error) {
                _this8.showAlert('Oops!', 'Unable to fetch channels', 'error');

                _this8.loadingChatList = false;
                _this8.loadingIsError = true;
              });
            }
          });
        } else {
    ...
    SetUnread: function SetUnread() {
      ...
      this.$root.ChatList.map(function (space) {
        var unreadStoredMsg = _this9.$root.getLocalStore('unread_messages_' + space.space_id + _this9.$root.username);
    ...
    checkIfisOwner: function checkIfisOwner() {
      ...
      var userMemberData = this.$root.selectedSpaceMembers.filter(function (members) {
        return members.user_id == _this10.$root.user_temp_id;
      });
        ...
        if (result != null) {
          ...
          if (finalResult.length != 0) {
            for (var index = 0; index < finalResult.length; index++) {
              _this11.$root.sendTextMessage(finalResult[index]);
            }

            _this11.$root.LocalStore('unsent_messages_' + _this11.$route.params.spaceId + _this11.$root.username, []);
          }
        } else {}
      });
    },
    makeUserMaster: function makeUserMaster(member) {
      ...
      axios.post('/make-user-master', {
        ...
        if (response.status == 200) {
          _this12.$root.adminMembers.forEach(function (member) {
            member.master_user = false;
          });

          _this12.$root.adminMembers.map(function (member) {
            if (member.memberId == response.data) {
              member.master_user = true;
            }
          });

          _this12.$root.selectedSpaceMembers.forEach(function (member) {
            member.master_user = false;
          });

          _this12.$root.selectedSpaceMembers.map(function (member) {
            if (member.memberId == response.data) {
              member.master_user = true;
            }
    ...
    autoMakeuserMaster: function autoMakeuserMaster() {
      ...
      if (this.checkIfisOwner()) {
        ...
        var userMember = this.$root.selectedSpaceMembers.filter(function (member) {
          return member.user_id == _this13.$root.user_temp_id;
        });
      ...
      axios.post('/check-for-unread-messages-clean', {
        ...
        if (response.status == 200) {
          if (response.data[0].length > 0) {
            var messageData = {
              space_id: _this14.$root.selectedSpace.space_id,
              new_messages: response.data[0]
            };

            _this14.$root.handleSpaceData([messageData]);
          }
        }
      })["catch"](function (error) {});
    },
    ...
    fetchMessages: function fetchMessages(spaceId) {
      ...
      if (this.$root.checkauthroot == 'auth') {
        ...
        storedMsg.then(function (result) {
          if (result != null) {
            var finalResult = JSON.parse(result);
            _this15.$root.spaceFullData = finalResult;

            var returnedData = _this15.handleResults(finalResult.messages);

            _this15.$root.Messages = returnedData;
            _this15.$root.selectedSpace = [];
            _this15.$root.selectedSpace = finalResult.space;

            if (finalResult.space.type == 'SubSpace') {
              var generalSpace = finalResult.space.gen_space;
              _this15.$root.selectedGenSpaceType = finalResult.space.gen_space.type;
              ...
              _this15.$root.selectedSubSpaceName = finalResult.space.name;
              _this15.$root.selectedSubSpaceType = subSpaceData.type;
              generalSpace.sub_spaces = finalResult.space.sub_spaces;
              _this15.$root.selectedSpace = generalSpace;
            }

            _this15.$root.TrackLastSubSpace.push(_this15.$root.selectedSpace.general_spaceId, _this15.$route.params.spaceId);

            _this15.$root.selectedSpaceMembers = finalResult.members; // set default admin user

            _this15.autoMakeuserMaster(); // get the message scroller component and scroll to bottom


            _this15.$root.msgScrollComponent = _this15.$refs;

            _this15.setShareLink(); // check for space in chatlist


            if (_this15.$root.selectedSpace.type == 'Channel' || _this15.$root.selectedSpace.type == 'Team' || _this15.$root.selectedSpace.type == 'Direct') {
              _this15.$root.addSpaceToChatList(_this15.$root.selectedSpace.space_id);
            } // generate unread msg and the mark as read


            _this15.generateUnreadMessage();

            setTimeout(function () {
              if (_this15.$root.selectedSpace.type == 'Channel' || _this15.$root.selectedSpace.type == 'Team') {
                if (_this15.$refs.messageContainer) {
                  _this15.$refs.messageContainer.scrollToBottom();
                }

                if (_this15.$refs.messageContainersmall) {
                  _this15.$refs.messageContainersmall.scrollToBottom();
                }
              } else {
                if (_this15.$refs.messageContainer) {
                  _this15.$refs.messageContainer.scrollToBottom();
            ...
            _this15.messageIsDone = true; // check for unsent messages and resend

            var unsentStoredMsg = _this15.$root.getLocalStore('unsent_messages_' + _this15.$route.params.spaceId + _this15.$root.username);

            unsentStoredMsg.then(function (result) {
              _this15.resendMessages();
            });

            _this15.fetchSpaceInfo();
          } else {
            axios.get('/fetch-space-messages-' + _this15.$route.params.spaceId).then(function (response) {
              if (response.status == 200) {
                if (response.data.space.space_id == _this15.$route.params.spaceId) {
                  _this15.$root.spaceFullData = response.data;

                  _this15.$root.LocalStore('full_' + _this15.$route.params.spaceId + _this15.$root.username, response.data);

                  _this15.$root.LocalStore('unread_messages_' + _this15.$route.params.spaceId + _this15.$root.username, []);

                  var _returnedData = _this15.handleResults(response.data.messages);

                  _this15.$root.Messages = _returnedData;
                  _this15.$root.selectedSpace = response.data.space;

                  if (response.data.space.type == 'SubSpace') {
                    var _generalSpace = response.data.space.gen_space;
                    _this15.$root.selectedGenSpaceType = response.data.space.gen_space.type;
                    ...
                    _this15.$root.selectedSubSpaceName = response.data.space.name;
                    _this15.$root.selectedSubSpaceType = _subSpaceData.type;
                    _generalSpace.sub_spaces = response.data.space.sub_spaces;
                    _this15.$root.selectedSpace = _generalSpace;
                  }

                  if (response.data.space.type == 'Bot') {
                    _this15.$root.botSuggestionArray = response.data.patterns;

                    _this15.$root.LocalStore('bot_latest_suggestions' + _this15.$root.selectedSpace.space_id + _this15.$root.username, response.data.patterns);
                  }

                  _this15.$root.TrackLastSubSpace.push(_this15.$root.selectedSpace.general_spaceId, _this15.$route.params.spaceId);

                  _this15.$root.selectedSpaceMembers = response.data.members;
                  _this15.$root.msgScrollComponent = _this15.$refs;
                  ...
                  _this15.messageIsDone = true;
                }
              }
            })["catch"](function (error) {
              _this15.errorLoadingMessage = true;
            });
          }

          if (_this15.$root.selectedSpace.type == 'Bot') {
            _this15.checkForNewNotes();
    ...
    checkForUnreadMessagesDisconnected: function checkForUnreadMessagesDisconnected() {
      ...
      if (this.chatIsOpen) {
        ...
        unreadStoredMsg.then(function (result) {
          ...
          if (_this16.$root.sendingMessage == false) {
            _this16.periodicUpdate(finalResultUnread);
          }
        });
      }
    },
    checkForNewNotes: function checkForNewNotes() {
      ...
      axios.get('/fetch-diary-notes-' + this.$root.selectedSpace.bot_data.bot_id).then(function (response) {
        if (response.status == 200) {
          _this17.$root.LocalStore('diary_notes_' + _this17.$root.selectedSpace.space_id + _this17.$root.username, response.data);

          var finalResult = response.data.diary_notes;
          _this17.$root.diaryNotes = finalResult;
        }
      })["catch"](function (error) {});
    },
    fetchSpaceInfo: function fetchSpaceInfo() {
      ...
      axios.get('/fetch-space-info-' + this.$root.selectedSpace.space_id).then(function (response) {
        if (response.status == 200) {
          var storedMsg = _this18.$root.getLocalStore('full_' + _this18.$root.selectedSpace.space_id + _this18.$root.username);
          ...
          storedMsg.then(function (result) {
            if (result != null) {
              ...
              _this18.$root.LocalStore('full_' + _this18.$root.selectedSpace.space_id + _this18.$root.username, finalResult);
            }
          }); // update chatList

          var storedChat = _this18.$root.getLocalStore('user_chat_list' + _this18.$root.username);
            ...
            if (result != null) {
              ...
              if (response.data.space.type == 'Channel' || response.data.space.type == 'Team') {
                finalResult.channels.map(function (chat) {
                  if (chat.space_id == _this18.$root.selectedSpace.space_id) {
              ...
              var fullList = finalResult.channels.concat(finalResult.direct_messages, finalResult.pet_spaces);
              _this18.$root.ChatList = fullList;

              _this18.$root.sortChatList();

              _this18.$root.LocalStore('user_chat_list' + _this18.$root.username, finalResult);
            }
          });
        }
      })["catch"](function (error) {});
      ...
      if (this.$root.Messages.length == 0) {} else {
        ...
        storedSuggestions.then(function (result) {
          ...
          if (finalResultUnread != null) {
            _this19.$root.botSuggestionArray = finalResultUnread;
          }
        });
      }
    }
              ...
              _c(
                ...
                [
                  ...
                  _vm.chatIsOpen && _vm.chatInnerConent == "image_viewer"
                    ...
                    : _vm._e(),
                  _vm._v(" "),
                  _vm.that.$root.selectedSpace.no_access
                    ? _c(
                        "div",
                        {
