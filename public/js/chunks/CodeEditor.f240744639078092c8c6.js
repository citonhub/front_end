  ...
  methods: {
    ...
    removeCode: function removeCode(codeBox) {
      ...
      var newarray = this.$root.codeEditorArray.filter(function (file) {
        if (newarray.length >= 1) {
          ...
          _this6.$router.push({
            path: '/board/projects/panel/' + _this6.$route.params.project_slug
          });
        }
      });
    ...
    saveFileContent: _.debounce(function () {
      ...
      axios.post('/save-code-content-project', {
        ...
        if (response.status == 200) {
          _this7.$root.codeEditorArray.map(function (codeFile) {
            if (codeFile.id == _this7.selectedFileId) {
              codeFile.content = _this7.code;
            }
          });

          if (_this7.$root.SelectedCodeBox.file_name == 'extensions') {
            var indexFile = _this7.$root.projectData.project_files.code_files.filter(function (codeFile) {
              return codeFile.file_name == 'index' && codeFile.language_type == 'HTML';
            });

            _this7.handleIndexFile(indexFile[0]);
          }

          _this7.loading = false;
        }
      })["catch"](function (error) {
        _this7.$root.projectPanelComponent.showAlert('Oops!', 'Unable to save changes', 'error');
    ...
    onCmCodeChange: function onCmCodeChange(code) {
      ...
      this.$root.projectData.project_files.code_files.map(function (file) {
        if (file.id == _this8.$root.selectedFileId) {
          file.content = code;
        }
      });
