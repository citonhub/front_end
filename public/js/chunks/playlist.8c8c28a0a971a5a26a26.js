    ...
    saveResourceOrder: function saveResourceOrder() {
      ...
      axios.post('/save-resources-order', {
        ...
        if (response.status == 200) {
          ...
          _this3.$root.LocalStore('channel_playlist_' + _this3.$root.selectedSpace.space_id + _this3.$root.username, _this3.playlists);
        }
      })["catch"](function (error) {
    ...
    fetchPlaylists: function fetchPlaylists() {
      ...
      storedPlaylist.then(function (result) {
        if (result != null) {
          ...
          _this4.playlists = finalResult;
          _this4.loadingPlaylist = false;

          _this4.checkForNewPlaylist();
        } else {
          axios.get("/fetch-resource/".concat(_this4.$root.selectedSpace.space_id, "/playlist")).then(function (response) {
            if (response.status == 200) {
              _this4.$root.LocalStore('channel_playlist_' + _this4.$root.selectedSpace.space_id + _this4.$root.username, response.data.resources);

              var _finalResult = response.data.resources;
              _this4.playlists = _finalResult;
              _this4.loadingPlaylist = false;
            }
          })["catch"](function (error) {
            _this4.loadingPlaylist = false;
    ...
    createPlaylists: function createPlaylists() {
      ...
      axios.post('/create-resource', {
        ...
        if (response.status == 200) {
          ...
          var storedPlaylist = _this6.$root.getLocalStore('channel_playlist_' + _this6.$root.selectedSpace.space_id + _this6.$root.username);
          ...
          storedPlaylist.then(function (result) {
            if (result != null) {
              ...
              _this6.$root.LocalStore('channel_playlist_' + _this6.$root.selectedSpace.space_id + _this6.$root.username, finalResult);
            } else {}
          });

          _this6.playlists.unshift(response.data.resource);
        }
      })["catch"](function (err) {
        console.log(err);
        _this6.saving = false;
                              ...
                              [
                                _c(
                                  "v-btn",
                                  {
                                    ...
                                    on: {
                                      click: function($event) {
                                        return _vm.deleteResource(playlist)
                                      }
                                    }
                                  },
                                  [_vm._v("Yes")]
