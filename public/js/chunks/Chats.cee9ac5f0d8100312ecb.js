...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    checkIfisOwner: function checkIfisOwner() {
      ...
      var userMemberData = this.$root.selectedSpaceMembers.filter(function (members) {
        return members.user_id == _this13.$root.user_temp_id;
      });
          ...
          (function () {
            ...
            if (finalResult.length != 0) {
              var _loop = function _loop(index) {
                setTimeout(function () {
                  _this14.$root.sendTextMessage(finalResult[index]);
                }, 2000);
              };
              ...
              _this14.$root.LocalStore('unsent_messages_' + _this14.$route.params.spaceId + _this14.$root.username, []);
            }
          })();
        } else {}
      });
    ...
    makeUserMaster: function makeUserMaster(member) {
      ...
      axios.post('/make-user-master', {
        ...
        if (response.status == 200) {
          _this15.$root.adminMembers.forEach(function (member) {
            member.master_user = false;
          });

          _this15.$root.adminMembers.map(function (member) {
            if (member.memberId == response.data) {
              member.master_user = true;
            }
          });

          _this15.$root.selectedSpaceMembers.forEach(function (member) {
            member.master_user = false;
          });

          _this15.$root.selectedSpaceMembers.map(function (member) {
            if (member.memberId == response.data) {
              member.master_user = true;
            }
    ...
    autoMakeuserMaster: function autoMakeuserMaster() {
      ...
      if (this.checkIfisOwner()) {
        ...
        var userMember = this.$root.selectedSpaceMembers.filter(function (member) {
          return member.user_id == _this16.$root.user_temp_id;
        });
      ...
      axios.post('/check-for-unread-messages-clean', {
        ...
        if (response.status == 200) {
          if (response.data[0].length > 0) {
            var messageData = {
              space_id: _this17.$root.selectedSpace.space_id,
              new_messages: response.data[0]
            };

            _this17.$root.handleSpaceData([messageData]);
          }
        }
      })["catch"](function (error) {});
    },
    ...
    fetchMessages: function fetchMessages(spaceId) {
      ...
      if (this.$root.checkauthroot == 'auth') {
        ...
        storedMsg.then(function (result) {
          if (result != null) {
            var finalResult = JSON.parse(result);
            _this18.$root.spaceFullData = finalResult;

            var returnedData = _this18.handleResults(finalResult.messages);

            _this18.$root.Messages = returnedData;
            _this18.$root.selectedSpace = [];
            _this18.$root.selectedSpace = finalResult.space;
            _this18.$root.selectedSpace.subspace_messages = 0;

            if (finalResult.space.type == 'SubSpace') {
              var generalSpace = finalResult.space.gen_space;
              _this18.$root.selectedGenSpaceType = finalResult.space.gen_space.type;
              ...
              _this18.$root.selectedSubSpaceName = finalResult.space.name;
              _this18.$root.selectedSubSpaceType = subSpaceData.type;
              generalSpace.sub_spaces = finalResult.space.sub_spaces;
              _this18.$root.selectedSpace = generalSpace;
            }

            _this18.$root.TrackLastSubSpace.push(_this18.$root.selectedSpace.general_spaceId, _this18.$route.params.spaceId);

            _this18.$root.selectedSpaceMembers = finalResult.members; // set default admin user

            _this18.autoMakeuserMaster(); // get the message scroller component and scroll to bottom


            _this18.$root.msgScrollComponent = _this18.$refs;

            _this18.setShareLink(); // check for space in chatlist


            if (_this18.$root.selectedSpace.type == 'Channel' || _this18.$root.selectedSpace.type == 'Team' || _this18.$root.selectedSpace.type == 'Direct') {
              _this18.$root.addSpaceToChatList(_this18.$root.selectedSpace.space_id);
            } // generate unread msg and the mark as read


            _this18.generateUnreadMessage(); // set up notification 


            _this18.$root.initialPushMangerReg(); // check unreadMessages in subspaces


            if (_this18.$root.selectedSpace.type == 'Channel' || _this18.$root.selectedSpace.type == 'Team' || _this18.$root.selectedSpace.type == 'SubSpace') {
              _this18.handleUnreadUpdate();
            }

            _this18.$root.updateSpaceMessages();

            setTimeout(function () {
              if (_this18.$root.selectedSpace.type == 'Channel' || _this18.$root.selectedSpace.type == 'Team') {
                if (_this18.$refs.messageContainer) {
                  _this18.$refs.messageContainer.scrollToBottom();
                }

                if (_this18.$refs.messageContainersmall) {
                  _this18.$refs.messageContainersmall.scrollToBottom();
                }
              } else {
                if (_this18.$refs.messageContainer) {
                  _this18.$refs.messageContainer.scrollToBottom();
            ...
            _this18.messageIsDone = true; // check for unsent messages and resend

            var unsentStoredMsg = _this18.$root.getLocalStore('unsent_messages_' + _this18.$route.params.spaceId + _this18.$root.username);

            unsentStoredMsg.then(function (result) {
              _this18.resendMessages();
            });

            _this18.fetchSpaceInfo();
          } else {
            axios.get('/fetch-space-messages-' + _this18.$route.params.spaceId).then(function (response) {
              if (response.status == 200) {
                if (response.data.space.space_id == _this18.$route.params.spaceId) {
                  _this18.$root.spaceFullData = response.data;

                  _this18.$root.LocalStore('full_space_' + _this18.$route.params.spaceId + _this18.$root.username, response.data);

                  _this18.$root.LocalStore('unread_messages_' + _this18.$route.params.spaceId + _this18.$root.username, []);

                  var _returnedData = _this18.handleResults(response.data.messages);

                  _this18.$root.Messages = _returnedData;
                  _this18.$root.selectedSpace = response.data.space;
                  _this18.$root.selectedSpace.subspace_messages = 0;

                  if (response.data.space.type == 'SubSpace') {
                    var _generalSpace = response.data.space.gen_space;
                    _this18.$root.selectedGenSpaceType = response.data.space.gen_space.type;
                    ...
                    _this18.$root.selectedSubSpaceName = response.data.space.name;
                    _this18.$root.selectedSubSpaceType = _subSpaceData.type;
                    _generalSpace.sub_spaces = response.data.space.sub_spaces;
                    _this18.$root.selectedSpace = _generalSpace;
                  }

                  if (response.data.space.type == 'Bot') {
                    _this18.$root.botSuggestionArray = response.data.patterns;

                    _this18.$root.LocalStore('bot_latest_suggestions' + _this18.$root.selectedSpace.space_id + _this18.$root.username, response.data.patterns);
                  ...
                  _this18.$root.selectedSpaceMembers = response.data.members;
                  _this18.$root.msgScrollComponent = _this18.$refs;
                  ...
                  _this18.$root.initialPushMangerReg();
                  ...
                  _this18.messageIsDone = true;
                }
              }
            })["catch"](function (error) {
              _this18.errorLoadingMessage = true;
            });
          }

          if (_this18.$root.selectedSpace.type == 'Bot') {
            _this18.checkForNewNotes();
    ...
    handleUnreadUpdate: function handleUnreadUpdate() {
      ...
      storedSubChat.then(function (result) {
        if (result != null) {
          ...
          if (_this19.checkIfisOwner()) {
            _this19.$root.subSpaces = finalResult;
          } else {
            _this19.$root.subSpaces = finalResult.filter(function (space) {
              return space.type == 'Public' || space.type == 'Private' && space.is_member == true;
            });
          }

          _this19.checkForUnreadSubSpace(_this19.$root.subSpaces, false);
        }
      });
    },
    checkForUnreadSubSpace: function checkForUnreadSubSpace(subSpaces) {
      ...
      subSpaces.map(function (space) {
        var unreadStoredMsg = _this20.$root.getLocalStore('unread_messages_' + space.space_id + _this20.$root.username);
        ...
        unreadStoredMsg.then(function (result) {
          ...
          if (returnCount) {
            ...
            _this20.$root.selectedSpaceSubMessages = fullCount;
          }
        });
      });
    },
    checkForUnreadMessagesDisconnected: function checkForUnreadMessagesDisconnected() {
      ...
      if (this.chatIsOpen) {
        ...
        unreadStoredMsg.then(function (result) {
          ...
          if (_this21.$root.sendingMessage == false) {
            _this21.periodicUpdate(finalResultUnread);
          }
        });
      }
    },
    checkForNewNotes: function checkForNewNotes() {
      ...
      axios.get('/fetch-diary-notes-' + this.$root.selectedSpace.bot_data.bot_id).then(function (response) {
        if (response.status == 200) {
          _this22.$root.LocalStore('diary_notes_' + _this22.$root.selectedSpace.space_id + _this22.$root.username, response.data);

          var finalResult = response.data.diary_notes;
          _this22.$root.diaryNotes = finalResult;
        }
      })["catch"](function (error) {});
    },
    fetchSpaceInfo: function fetchSpaceInfo() {
      ...
      axios.get('/fetch-space-info-' + this.$root.selectedSpace.space_id).then(function (response) {
        if (response.status == 200) {
          var storedMsg = _this23.$root.getLocalStore('full_space_' + _this23.$root.selectedSpace.space_id + _this23.$root.username);
          ...
          storedMsg.then(function (result) {
            if (result != null) {
              ...
              if (response.data.space.type == 'SubSpace') {
                ...
                _this23.$root.selectedGenSpaceType = response.data.space.gen_space.type;
                ...
                _this23.$root.selectedSubSpaceName = response.data.space.name;
                _this23.$root.selectedSubSpaceType = subSpaceData.type;
                generalSpace.sub_spaces = response.data.space.sub_spaces;
                _this23.$root.selectedSpace = generalSpace;
              } else {
                _this23.$root.selectedSpace = response.data.space;
              }

              finalResult.members = response.data.members;

              _this23.$root.LocalStore('full_space_' + _this23.$root.selectedSpace.space_id + _this23.$root.username, finalResult);
            }
          }); // update chatList

          var storedChat = _this23.$root.getLocalStore('user_chat_list_new_' + _this23.$root.username);
            ...
            if (result != null) {
              ...
              if (response.data.space.type == 'Channel' || response.data.space.type == 'Team') {
                finalResult.channels.map(function (chat) {
                  if (chat.space_id == _this23.$root.selectedSpace.space_id) {
              ...
              var fullList = finalResult.channels.concat(finalResult.direct_messages, finalResult.pet_spaces);
              _this23.$root.ChatList = fullList;

              _this23.$root.sortChatList();

              _this23.SetUnread();

              _this23.$root.LocalStore('user_chat_list_new_' + _this23.$root.username, finalResult);
            }
          });
        }
      })["catch"](function (error) {});
