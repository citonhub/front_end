    ...
    fetchPost: function fetchPost() {
      ...
      storedPostData.then(function (result) {
        if (result != null) {
          ...
          _this3.$root.selectedPost = finalResult;
          _this3.loadingPost = false; // this.updatePost();
        } else {
          axios.get('/fetch-post/' + _this3.$route.params.post_id).then(function (response) {
            if (response.status == 200) {
              _this3.$root.LocalStore('post_data_' + _this3.$route.params.post_id + _this3.$root.username, response.data.data);

              _this3.$root.selectedPost = response.data.data;
              _this3.loadingPost = false;
            }
          })["catch"](function (error) {
            _this3.loadingPost = false;
          });
