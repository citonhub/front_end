...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: (_methods = {
    ...
    fetchComments: function fetchComments(postId) {
      ...
      axios.get('/comments/' + postId).then(function (response) {
        if (response.status == 200) {
          _this2.$root.postCommentsData = response.data.data;

          _this2.$root.postCommentsData.map(function (eachcomment) {
    ...
    scrollToComment: function scrollToComment(comment) {
      ...
      var commentData = this.$root.postCommentsData.filter(function (eachcomment) {
        return comment.id == eachcomment.id;
      });

      if (commentData.length > 0) {
        var fullComment = commentData[0];
        var commentIndex = this.$root.postCommentsData.indexOf(fullComment);
        this.$refs.commentScrollerPost.scrollToItem(commentIndex);
        this.$root.postCommentsData.map(function (eachcomment) {
          if (comment.id == eachcomment.id) {
            eachcomment.tagged = true;
          }
        });
        setTimeout(function () {
          _this3.$root.postCommentsData.map(function (eachcomment) {
            if (comment.id == eachcomment.id) {
              eachcomment.tagged = false;
            }
          });
    ...
    likeComment: function likeComment(comment) {
      ...
      axios.post('/like-hub-post-comment', {
        ...
        if (response.status == 200) {
          _this4.$root.postCommentsData.map(function (eachcomment) {
      ...
      if (this.commentValue != '') {
        ...
        axios.post('/comment-hub-post', formData).then(function (response) {
          if (response.status == 201) {
            ...
            _this5.$root.postCommentsData.push(Newcomment);
    ...
    storedPostData.then(function (result) {
      if (result != null) {
        ...
        axios.get('/fetch-post/' + _this10.$route.params.post_id).then(function (response) {
          if (response.status == 200) {
          ...
          _c(
            ...
            [
              _vm.loadingPostComments
                ...
                : [
                    _vm.that.$root.postCommentsData.length == 0
                      ? [_vm._m(0)]
                      : [
                          _c("DynamicScroller", {
                            ...
                            attrs: {
                              items: _vm.that.$root.postCommentsData,
                              keyField: "id",
                              "min-item-size": 36,
                              buffer: 5000,
