  ...
  methods: {
    ...
    vidoehandler: function vidoehandler(e) {
      ...
      if (fileSize <= 100000000) {
        fr.addEventListener('load', function () {
          _this12.videoUrl = fr.result; // send message
    ...
    sendMessageData: function sendMessageData(formData) {
      ...
      axios.post('/create-response-content', formData, {
        ...
        onUploadProgress: function onUploadProgress(progressEvent) {
          _this13.progressvalue = parseInt(Math.round(progressEvent.loaded / progressEvent.total * 100));
        }
      }).then(function (response) {
        if (response.status == 200) {
          var message = response.data.message;

          _this13.$root.noteContent.pages[_this13.selectedContentId].contents.push(message);

          _this13.saveContentOrder(false);

          _this13.clearData();

          _this13.loadingSendMsg = false;
          _this13.progressvalue = 0;
          _this13.$root.AddModalIsUp = false;
          _this13.$root.selectedDiary.updated = false;
        }
      })["catch"](function (error) {
        _this13.showAlert('Oops!', 'Something went wrong, please try again', 'error');
