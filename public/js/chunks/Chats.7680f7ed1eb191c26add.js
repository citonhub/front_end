...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    searchChatList: function searchChatList() {
      ...
      this.searchChatList = chatListResult;
    },
        ...
        if (this.$root.ChatList.length == 0) {
          ...
          storedChat.then(function (result) {
            if (result != null) {
              ...
              _this2.$root.baseChatList = finalResult;
              ...
              axios.get('/fetch-chat-list/' + _this2.$root.userDeviceId).then(function (response) {
                ...
                _this2.showAlert('Oops!', 'Unable to fetch channels', 'error');

                _this2.loadingChatList = false;
                _this2.loadingIsError = true;
              });
            }
          });
        }
    ...
    SetUnread: function SetUnread() {
      ...
      this.$root.ChatList.map(function (space) {
        var unreadStoredMsg = _this3.$root.getLocalStore('unread_messages_' + space.space_id + _this3.$root.username);
        ...
        if (result != null) {
          ...
          if (finalResult.length == 0) {} else {
            for (var index = 0; index < finalResult.length; index++) {
              if (_this5.$route.params.spaceId == finalResult[index].space_id) {
                _this5.$root.sendTextMessage(finalResult[index]);
              }
            }

            _this5.$root.LocalStore('unsent_messages_' + _this5.$route.params.spaceId + _this5.$root.username, []);
          }
        } else {}
      });
    },
    makeUserMaster: function makeUserMaster(member) {
      ...
      axios.post('/make-user-master', {
        ...
        if (response.status == 200) {
          _this6.$root.adminMembers.forEach(function (member) {
            member.master_user = false;
          });

          _this6.$root.adminMembers.map(function (member) {
            if (member.memberId == response.data) {
              member.master_user = true;
            }
          });

          _this6.$root.selectedSpaceMembers.forEach(function (member) {
            member.master_user = false;
          });

          _this6.$root.selectedSpaceMembers.map(function (member) {
            if (member.memberId == response.data) {
              member.master_user = true;
            }
    ...
    autoMakeuserMaster: function autoMakeuserMaster() {
      ...
      if (this.checkIfisOwner()) {
        ...
        var userMember = this.$root.selectedSpaceMembers.filter(function (member) {
          return member.user_id == _this7.$root.user_temp_id;
        });
        ...
        if (response.status == 200) {
          if (response.data[0].length > 0) {
            for (var index = 0; index < response.data[0].length; index++) {
              response.data[0][index].index_count = _this8.$root.returnLastIndex() + 1;
              response.data[0][index].id = response.data[0][index].message_id;
              response.data[0][index].initialSize = 200;

              _this8.$root.Messages.push(response.data[0][index]);

              _this8.$root.pushDataToLocal(response.data[0][index]);
    ...
    fetchMessages: function fetchMessages(spaceId) {
      ...
      if (this.$root.checkauthroot == 'auth') {
        ...
        storedMsg.then(function (result) {
          if (result != null) {
            var finalResult = JSON.parse(result);
            _this9.$root.spaceFullData = finalResult;

            var returnedData = _this9.handleResults(finalResult.messages);

            _this9.$root.Messages = returnedData;
            _this9.$root.selectedSpace = []; // generate unread msg and the mark as read

            _this9.generateUnreadMessage();

            _this9.$root.selectedSpace = finalResult.space;

            if (finalResult.space.type == 'SubSpace') {
              var generalSpace = finalResult.space.gen_space;
              _this9.$root.selectedGenSpaceType = finalResult.space.gen_space.type;
              ...
              _this9.$root.selectedSubSpaceName = finalResult.space.name;
              _this9.$root.selectedSubSpaceType = subSpaceData.type;
              generalSpace.sub_spaces = finalResult.space.sub_spaces;
              _this9.$root.selectedSpace = generalSpace;
            }

            _this9.$root.TrackLastSubSpace.push(_this9.$root.selectedSpace.general_spaceId, _this9.$route.params.spaceId);

            _this9.$root.selectedSpaceMembers = finalResult.members; // set default admin user

            _this9.autoMakeuserMaster(); // get the message scroller component and scroll to bottom


            _this9.$root.msgScrollComponent = _this9.$refs;
            setTimeout(function () {
              if (_this9.$root.selectedSpace.type == 'Channel' || _this9.$root.selectedSpace.type == 'Team') {
                _this9.$root.msgScrollComponent.messageContainer.scrollToBottom();

                _this9.$refs.messageContainersmall.scrollToBottom();
              } else {
                _this9.$root.msgScrollComponent.messageContainer.scrollToBottom();
            ...
            setTimeout(function () {
              _this9.messageIsDone = true;
            }, 2500);

            var unreadStoredMsg = _this9.$root.getLocalStore('unread_messages_' + _this9.$route.params.spaceId + _this9.$root.username);

            unreadStoredMsg.then(function (result) {
              var finalResultUnread = JSON.parse(result);

              if (_this9.$root.sendingMessage == false) {
                _this9.periodicUpdate(finalResultUnread);
              }
            }); // check for unsent messages and resend

            var unsentStoredMsg = _this9.$root.getLocalStore('unsent_messages_' + _this9.$route.params.spaceId + _this9.$root.username);
            ...
            axios.get('/fetch-space-messages-' + _this9.$route.params.spaceId).then(function (response) {
              if (response.status == 200) {
                if (response.data.space.space_id == _this9.$route.params.spaceId) {
                  _this9.$root.spaceFullData = response.data;

                  _this9.$root.LocalStore('full_' + _this9.$route.params.spaceId + _this9.$root.username, response.data);

                  _this9.$root.LocalStore('unread_messages_' + _this9.$route.params.spaceId + _this9.$root.username, []);

                  var _returnedData = _this9.handleResults(response.data.messages);

                  _this9.$root.Messages = _returnedData;
                  _this9.$root.selectedSpace = response.data.space;

                  if (response.data.space.type == 'SubSpace') {
                    var _generalSpace = response.data.space.gen_space;
                    _this9.$root.selectedGenSpaceType = response.data.space.gen_space.type;
                    ...
                    _this9.$root.selectedSubSpaceName = response.data.space.name;
                    _this9.$root.selectedSubSpaceType = _subSpaceData.type;
                    _generalSpace.sub_spaces = response.data.space.sub_spaces;
                    _this9.$root.selectedSpace = _generalSpace;
                  }

                  if (response.data.space.type == 'Bot') {
                    _this9.botSuggestionArray = response.data.patterns;

                    _this9.$root.LocalStore('bot_latest_suggestions' + _this9.$root.selectedSpace.space_id + _this9.$root.username, response.data[1]);
                  }

                  _this9.$root.TrackLastSubSpace.push(_this9.$root.selectedSpace.general_spaceId, _this9.$route.params.spaceId);

                  _this9.$root.selectedSpaceMembers = response.data.members;
                  _this9.$root.msgScrollComponent = _this9.$refs;
                  setTimeout(function () {
                    _this9.$root.msgScrollComponent.messageContainer.scrollToBottom();
              ...
              _this9.errorLoadingMessage = true;
            });
          }

          if (_this9.$root.selectedSpace.type == 'Bot') {
            setTimeout(function () {
              _this9.botMessager();
            }, 2000);
          }

          if (_this9.$root.selectedSpace.general_spaceId != undefined) {//      this.getMemberUpdates(this.$root.selectedSpace.general_spaceId);
          }
        });
      }
    }
  ...
  getMemberUpdates: function getMemberUpdates(spaceId) {
    ...
    axios.get('/fetch-space-members-' + spaceId).then(function (response) {
      if (response.status == 200) {
        _this11.$root.selectedSpaceMembers = response.data;
      }
    })["catch"](function (error) {});
  },
    ...
    if (this.$root.Messages.length == 0) {} else {
      ...
      storedSuggestions.then(function (result) {
        ...
        if (finalResultUnread != null) {
          _this12.$root.botSuggestionArray = finalResultUnread;
        }
      });
    }
  }
    ...
    return _c(
      ...
      [
        ...
        _c(
          ...
          {
            staticClass: "text-center",
            staticStyle: { "font-family": "HeaderFont", "font-size": "15px" }
          },
          [
            _vm._v(
              "\n                               - Rob Siltanen\n                            "
