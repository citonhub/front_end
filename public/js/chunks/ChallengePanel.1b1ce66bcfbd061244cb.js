  ...
  methods: {
    ...
    createProject: function createProject() {
      ...
      var language = this.languageIcon.filter(function (app) {
        return app.id == _this3.$root.selectedChallenge.duel_language_array;
      });
      ...
      axios.post('/create-project', {
        ...
        if (response.status == 200) {
          _this3.joinChallenge(response.data.project);
        }
      })["catch"](function (error) {
    ...
    reloadChallenge: function reloadChallenge() {
      ...
      axios.get('/fetch-challenge-data/' + this.$route.params.challenge_id).then(function (response) {
        if (response.status == 200) {
          ...
          _this5.countDownDate = new Date(_this5.$root.selectedChallenge.duel_terminal_time).getTime();

          var status = _this5.checkDuelStatus(_this5.$root.selectedChallenge);
    ...
    fetchChallenge: function fetchChallenge() {
      ...
      storedProjectData.then(function (result) {
        if (result != null) {
          var finalResult = JSON.parse(result);
          _this6.$root.selectedChallenge = finalResult;
          ...
          _this6.loadingChallenge = false; // this.checkForNewChallengeData();
        } else {
          axios.get('/fetch-challenge-data/' + _this6.$route.params.challenge_id).then(function (response) {
            if (response.status == 200) {
