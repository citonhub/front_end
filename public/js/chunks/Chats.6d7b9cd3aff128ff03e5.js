...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
        ...
        if (this.$root.ChatList.length == 0) {
          ...
          storedChat.then(function (result) {
            if (result != null) {
              ...
              fullList.forEach(function (space) {
    ...
    getUnread: function getUnread(space_id) {
      var unreadStoredMsg = this.$root.getLocalStore('unread_messages_' + 'space' + this.$root.username);
      unreadStoredMsg.then(function (result) {
        if (result != null) {
          var finalResultUnread = JSON.parse(result);
          return console.log(finalResultUnread.length);
        } else {
