  ...
  methods: (_methods = {
    monitorLink: function monitorLink() {
      ...
      var ife = setInterval(function () {
        var frameElement = document.getElementById('mainIframe');

        if (frameElement) {
          frameElement.load(function (ev) {
            ...
            try {
              ...
              that.supportedLink = false; //I opened the url in new page
      ...
      if (this.commentValue != '') {
        ...
        axios.post('/comment-hub-post', formData).then(function (response) {
          if (response.status == 201) {
            ...
            _this5.comments.unshift(Newcomment);
    ...
    axios.post('/run-code-on-sandbox-project', {
      ...
      if (response.status == 200) {
        ...
        if (response.data[0][0].status.description == 'Accepted') {
          ...
          _this9.pageContent = response.data[0].stdout + '\n Error: \n' + response.data[0].stderr + ' \n' + response.data[0].compile_output + '\n' + response.data[0].error;
        }
      }
    })["catch"](function (error) {
    ...
    storedPostData.then(function (result) {
      if (result != null) {
        ...
        _this10.panelData = _this10.$root.selectedPost.project.project_panel;
        ...
        axios.get('/fetch-post/' + currentPostid).then(function (response) {
          if (response.status == 200) {
            _this10.$root.LocalStore('post_data_new_' + currentPostid + _this10.$root.username, response.data.data);
    ...
    axios.get('/fetch-post/' + this.$route.params.post_id).then(function (response) {
      if (response.status == 200) {
        _this11.$root.LocalStore('post_data_new_' + _this11.$route.params.post_id + _this11.$root.username, response.data.data);
                      ...
                      directives: [
                        {
                          name: "show",
                          rawName: "v-show",
                          value: _vm.supportedLink,
                          expression: "supportedLink"
                        }
                      ],
                        ...
                        _c(
                          ...
                          [
                            ...
                            _c(
                              ...
                              {
                                attrs: {
                                  href: this.$root.selectedPost.project_url
                                }
                              },
                              [_vm._v("Open link ")]
                            )
