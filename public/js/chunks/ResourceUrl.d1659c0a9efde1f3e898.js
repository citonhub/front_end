...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    addToResource: function addToResource() {
      ...
      var itemType = 'shared_link';
      var dataToSend = [this.linkInfo];
      ...
      axios.post('/add-items-to-resource', {
        ...
        if (response.status == 200) {
          ...
          _this2.$root.resourcesData = responseContent.concat(_this2.$root.resourcesData);
    ...
    saveOrder: function saveOrder() {
      ...
      axios.post('/save-resources-content-order', {
        ...
        if (response.status == 200) {
          _this3.$root.LocalStore('channel_resource_content_' + _this3.$root.selectedResource.resource_id + _this3.$root.username, _this3.$root.resourcesData);

          _this3.$root.loadingResourceContent = false;
        }
      })["catch"](function (error) {
    ...
    getVideoData: function getVideoData(videoId) {
      ...
      axios.post('/get-video-data', {
        ...
        if (response.status == 200) {
          var videoData = [response.data.video_data];
          _this4.videoData = [response.data.video_data];
          _this4.isYoutube = true;
          var finalResult = [];
    ...
    getLinkInfo: function getLinkInfo(e) {
      ...
      if (this.isURL(this.searchQuery)) {
        var urlDomain = this.processURL(this.searchQuery);

        if (urlDomain == 'youtu.be') {
          var videoId = this.searchQuery.substring(17);
        ...
        axios.post('/url/metadata', {
          ...
          if (response.status == 200) {
            _this5.linkInfo = response.data;
            _this5.loadingLink = false;
          }
        })["catch"](function (err) {
          console.log(err);

          _this5.$root.chatComponent.showAlert('Oops!', 'Unable to fetch URL details', 'error', 'bottomRight', 3000);
                  ...
                  _vm.isYoutube
                    ? [
                        _c(
                          ...
                          {
                            staticClass:
                              "col-12  pb-0 pt-0 px-2 d-flex flex-row mt-1",
                            ...
                            _c(
                              ...
                              [
                                _c("div", {
                                  ...
                                  domProps: {
                                    innerHTML: _vm._s(_vm.linkInfo.title)
                                  }
                                }),
                                _vm._v(" "),
                                _c("div", {
                                ...
                                _c(
                                  ...
                                  [
                                    _vm._v(
                                      "\n            " +
                                        _vm._s(
                                          _vm.processURL(_vm.linkInfo.base)
                                        ) +
                                        "\n           "
                                    )
                                  ]
