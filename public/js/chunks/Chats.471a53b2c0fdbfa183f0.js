  ...
  methods: {
    ...
    showSideBarHandler: function showSideBarHandler() {
      ...
      if (!this.$root.authProfile.user_onboarded) {
        axios.post('/save-user-onboarded-status').then(function (response) {
          if (response.status == 200) {
            _this4.$root.authProfile.user_onboarded = true;
          }
        })["catch"](function (error) {});
      }
    },
    checkDiary: function checkDiary() {
      ...
      axios.get('/check-diary/' + botId).then(function (response) {
        if (response.status == 200) {
          ...
          var storedChat = _this5.$root.getLocalStore('user_chat_list' + _this5.$root.username);
          ...
          storedChat.then(function (result) {
            if (result != null) {
              ...
              if (userSpace.length > 0) {} else {
                ...
                _this5.$root.LocalStore('user_chat_list' + _this5.$root.username, finalResult);

                var fullList = finalResult.channels.concat(finalResult.direct_messages, finalResult.pet_spaces);
                _this5.$root.ChatList = fullList;

                _this5.$root.sortChatList();
              }

              _this5.openChat(response.data.space.space_id, true);

              _this5.$root.showDiarySettings = false;
            }
          });
          _this5.$root.showDiarySettings = false;
        }
    ...
    searchChatList: function searchChatList() {
      ...
      var chatListResult = this.$root.ChatList.filter(function (chat) {
        ...
        return nameValue.includes(_this6.searchValue.toLowerCase());
      });
      ...
      if (this.$root.checkauthroot == 'auth') {
        if (this.$root.ChatList.length == 0) {
          ...
          storedChat.then(function (result) {
            if (result != null) {
              var finalResult = JSON.parse(result);
              _this7.$root.baseChatList = finalResult;
              _this7.$root.channelChats = finalResult.channels;
              var fullList = finalResult.channels.concat(finalResult.direct_messages, finalResult.pet_spaces);
              _this7.$root.ChatList = fullList;

              _this7.$root.sortChatList();

              _this7.SetUnread(); // get all new messages


              _this7.$root.updateSpaceMessages();

              _this7.$root.loadingChatList = false;
            } else {
              axios.get('/fetch-chat-list/' + _this7.$root.userDeviceId).then(function (response) {
                if (response.status == 200) {
                  var responseList = response.data;
                  _this7.$root.baseChatList = response.data;
                  _this7.$root.channelChats = responseList.channels;
                  _this7.$root.ChatList = responseList.channels.concat(responseList.direct_messages, responseList.pet_spaces);

                  _this7.$root.LocalStore('user_chat_list' + _this7.$root.username, response.data, false, 'sort_chat');

                  _this7.$root.loadingChatList = false;
                }
              })["catch"](function (error) {
                _this7.showAlert('Oops!', 'Unable to fetch channels', 'error');

                _this7.loadingChatList = false;
                _this7.loadingIsError = true;
              });
            }
          });
        } else {
    ...
    SetUnread: function SetUnread() {
      ...
      this.$root.ChatList.map(function (space) {
        var unreadStoredMsg = _this8.$root.getLocalStore('unread_messages_' + space.space_id + _this8.$root.username);
    ...
    checkIfisOwner: function checkIfisOwner() {
      ...
      var userMemberData = this.$root.selectedSpaceMembers.filter(function (members) {
        return members.user_id == _this9.$root.user_temp_id;
      });
        ...
        if (result != null) {
          ...
          if (finalResult.length != 0) {
            for (var index = 0; index < finalResult.length; index++) {
              _this10.$root.sendTextMessage(finalResult[index]);
            }

            _this10.$root.LocalStore('unsent_messages_' + _this10.$route.params.spaceId + _this10.$root.username, []);
          }
        } else {}
      });
    },
    makeUserMaster: function makeUserMaster(member) {
      ...
      axios.post('/make-user-master', {
        ...
        if (response.status == 200) {
          _this11.$root.adminMembers.forEach(function (member) {
            member.master_user = false;
          });

          _this11.$root.adminMembers.map(function (member) {
            if (member.memberId == response.data) {
              member.master_user = true;
            }
          });

          _this11.$root.selectedSpaceMembers.forEach(function (member) {
            member.master_user = false;
          });

          _this11.$root.selectedSpaceMembers.map(function (member) {
            if (member.memberId == response.data) {
              member.master_user = true;
            }
    ...
    autoMakeuserMaster: function autoMakeuserMaster() {
      ...
      if (this.checkIfisOwner()) {
        ...
        var userMember = this.$root.selectedSpaceMembers.filter(function (member) {
          return member.user_id == _this12.$root.user_temp_id;
        });
      ...
      axios.post('/check-for-unread-messages-clean', {
        ...
        if (response.status == 200) {
          if (response.data[0].length > 0) {
            var messageData = {
              space_id: _this13.$root.selectedSpace.space_id,
              new_messages: response.data[0]
            };

            _this13.$root.handleSpaceData([messageData]);
          }
        }
      })["catch"](function (error) {});
    },
    ...
    fetchMessages: function fetchMessages(spaceId) {
      ...
      if (this.$root.checkauthroot == 'auth') {
        ...
        storedMsg.then(function (result) {
          if (result != null) {
            var finalResult = JSON.parse(result);
            _this14.$root.spaceFullData = finalResult;

            var returnedData = _this14.handleResults(finalResult.messages);

            _this14.$root.Messages = returnedData;
            _this14.$root.selectedSpace = [];
            _this14.$root.selectedSpace = finalResult.space;

            if (finalResult.space.type == 'SubSpace') {
              var generalSpace = finalResult.space.gen_space;
              _this14.$root.selectedGenSpaceType = finalResult.space.gen_space.type;
              ...
              _this14.$root.selectedSubSpaceName = finalResult.space.name;
              _this14.$root.selectedSubSpaceType = subSpaceData.type;
              generalSpace.sub_spaces = finalResult.space.sub_spaces;
              _this14.$root.selectedSpace = generalSpace;
            }

            _this14.$root.TrackLastSubSpace.push(_this14.$root.selectedSpace.general_spaceId, _this14.$route.params.spaceId);

            _this14.$root.selectedSpaceMembers = finalResult.members; // set default admin user

            _this14.autoMakeuserMaster(); // get the message scroller component and scroll to bottom


            _this14.$root.msgScrollComponent = _this14.$refs;

            _this14.setShareLink(); // check for space in chatlist


            if (_this14.$root.selectedSpace.type == 'Channel' || _this14.$root.selectedSpace.type == 'Team' || _this14.$root.selectedSpace.type == 'Direct') {
              _this14.$root.addSpaceToChatList(_this14.$root.selectedSpace.space_id);
            } // generate unread msg and the mark as read


            _this14.generateUnreadMessage();

            setTimeout(function () {
              if (_this14.$root.selectedSpace.type == 'Channel' || _this14.$root.selectedSpace.type == 'Team') {
                if (_this14.$refs.messageContainer) {
                  _this14.$refs.messageContainer.scrollToBottom();
                }

                if (_this14.$refs.messageContainersmall) {
                  _this14.$refs.messageContainersmall.scrollToBottom();
                }
              } else {
                if (_this14.$refs.messageContainer) {
                  _this14.$refs.messageContainer.scrollToBottom();
            ...
            _this14.messageIsDone = true; // check for unsent messages and resend

            var unsentStoredMsg = _this14.$root.getLocalStore('unsent_messages_' + _this14.$route.params.spaceId + _this14.$root.username);

            unsentStoredMsg.then(function (result) {
              _this14.resendMessages();
            });

            _this14.fetchSpaceInfo();
          } else {
            axios.get('/fetch-space-messages-' + _this14.$route.params.spaceId).then(function (response) {
              if (response.status == 200) {
                if (response.data.space.space_id == _this14.$route.params.spaceId) {
                  _this14.$root.spaceFullData = response.data;

                  _this14.$root.LocalStore('full_' + _this14.$route.params.spaceId + _this14.$root.username, response.data);

                  _this14.$root.LocalStore('unread_messages_' + _this14.$route.params.spaceId + _this14.$root.username, []);

                  var _returnedData = _this14.handleResults(response.data.messages);

                  _this14.$root.Messages = _returnedData;
                  _this14.$root.selectedSpace = response.data.space;

                  if (response.data.space.type == 'SubSpace') {
                    var _generalSpace = response.data.space.gen_space;
                    _this14.$root.selectedGenSpaceType = response.data.space.gen_space.type;
                    ...
                    _this14.$root.selectedSubSpaceName = response.data.space.name;
                    _this14.$root.selectedSubSpaceType = _subSpaceData.type;
                    _generalSpace.sub_spaces = response.data.space.sub_spaces;
                    _this14.$root.selectedSpace = _generalSpace;
                  }

                  if (response.data.space.type == 'Bot') {
                    _this14.$root.botSuggestionArray = response.data.patterns;

                    _this14.$root.LocalStore('bot_latest_suggestions' + _this14.$root.selectedSpace.space_id + _this14.$root.username, response.data.patterns);
                  }

                  _this14.$root.TrackLastSubSpace.push(_this14.$root.selectedSpace.general_spaceId, _this14.$route.params.spaceId);

                  _this14.$root.selectedSpaceMembers = response.data.members;
                  _this14.$root.msgScrollComponent = _this14.$refs;
                  ...
                  _this14.messageIsDone = true;
                }
              }
            })["catch"](function (error) {
              _this14.errorLoadingMessage = true;
            });
          }

          if (_this14.$root.selectedSpace.type == 'Bot') {
            _this14.checkForNewNotes();
    ...
    checkForUnreadMessagesDisconnected: function checkForUnreadMessagesDisconnected() {
      ...
      if (this.chatIsOpen) {
        ...
        unreadStoredMsg.then(function (result) {
          ...
          if (_this15.$root.sendingMessage == false) {
            _this15.periodicUpdate(finalResultUnread);
          }
        });
      }
    },
    checkForNewNotes: function checkForNewNotes() {
      ...
      axios.get('/fetch-diary-notes-' + this.$root.selectedSpace.bot_data.bot_id).then(function (response) {
        if (response.status == 200) {
          _this16.$root.LocalStore('diary_notes_' + _this16.$root.selectedSpace.space_id + _this16.$root.username, response.data);

          var finalResult = response.data.diary_notes;
          _this16.$root.diaryNotes = finalResult;
        }
      })["catch"](function (error) {});
    },
    fetchSpaceInfo: function fetchSpaceInfo() {
      ...
      axios.get('/fetch-space-info-' + this.$root.selectedSpace.space_id).then(function (response) {
        if (response.status == 200) {
          var storedMsg = _this17.$root.getLocalStore('full_' + _this17.$root.selectedSpace.space_id + _this17.$root.username);
          ...
          storedMsg.then(function (result) {
            if (result != null) {
              ...
              _this17.$root.LocalStore('full_' + _this17.$root.selectedSpace.space_id + _this17.$root.username, finalResult);
            }
          }); // update chatList

          var storedChat = _this17.$root.getLocalStore('user_chat_list' + _this17.$root.username);
            ...
            if (result != null) {
              ...
              if (response.data.space.type == 'Channel' || response.data.space.type == 'Team') {
                finalResult.channels.map(function (chat) {
                  if (chat.space_id == _this17.$root.selectedSpace.space_id) {
              ...
              var fullList = finalResult.channels.concat(finalResult.direct_messages, finalResult.pet_spaces);
              _this17.$root.ChatList = fullList;

              _this17.$root.sortChatList();

              _this17.$root.LocalStore('user_chat_list' + _this17.$root.username, finalResult);
            }
          });
        }
      })["catch"](function (error) {});
      ...
      if (this.$root.Messages.length == 0) {} else {
        ...
        storedSuggestions.then(function (result) {
          ...
          if (finalResultUnread != null) {
            _this18.$root.botSuggestionArray = finalResultUnread;
          }
        });
      }
    }
