  ...
  methods: {
    ...
    saveSettings: function saveSettings() {
      ...
      axios.post('/save-panel-settings', {
        ...
        if (response.status == 200) {
          ...
          _this.$router.push({
            path: '/board/projects/panel/' + _this.$root.projectData.project.project_slug
          });
        }
    ...
    fetchConnected: function fetchConnected() {
      ...
      axios.get('/fetch-connected').then(function (response) {
        if (response.status == 200) {
          _this2.Connections = response.data.data; // get existing contributors

          var contributors = [];

          _this2.$root.ProjectMembers.forEach(function (member) {
            _this2.Connections.forEach(function (connected) {
          ...
          _this2.contributors = contributors;
