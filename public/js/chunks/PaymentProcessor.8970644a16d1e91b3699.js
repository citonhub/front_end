    ...
    fetchSpaceInfo: function fetchSpaceInfo() {
      ...
      axios.get('/fetch-space-info-' + this.$route.params.spaceId).then(function (response) {
        if (response.status == 200) {
          ...
          if (_this2.spaceData.type != 'Channel' && _this2.spaceData.type != 'Team' && _this2.spaceData.type != 'SubSpace') {
            _this2.$root.chatComponent.openChat(_this2.$route.params.spaceId, true);

            return;
          }

          if (_this2.spaceData.is_member) {
            if (!_this2.$root.showProcessorFromChat) {
              _this2.$root.chatComponent.openChat(_this2.$route.params.spaceId, true);

              return;
            } else {}
          } else {
            if (_this2.spaceData.payment_option == 'support' && _this2.$root.isLogged) {
              if (!_this2.$root.fromSupportDirectlink) {
                _this2.$root.chatComponent.openChat(_this2.$route.params.spaceId, false);
          ...
          if (_this2.spaceData.payment_option == 'subscription') {
            _this2.currency = _this2.spaceData.payment_data.currency;
            _this2.interval = _this2.spaceData.payment_data.interval;
            _this2.amount = _this2.spaceData.payment_data.amount;
            _this2.supporter_name = 'supporter';
          }

          if (_this2.spaceData.payment_option == 'one_time') {
            _this2.currency = _this2.spaceData.payment_data.currency;
            _this2.amount = _this2.spaceData.payment_data.amount;
            _this2.supporter_name = 'supporter';
          }

          if (_this2.$root.isLogged) {
            _this2.supporter_name = _this2.$root.authProfile.name;
          }
