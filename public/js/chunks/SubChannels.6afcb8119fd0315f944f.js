...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    fetchSubSpaces: function fetchSubSpaces() {
      ...
      storedSubChat.then(function (result) {
        if (result != null) {
          ...
          if (_this.checkIfisOwner()) {
            _this.subSpaces = finalResult;
          } else {
            _this.subSpaces = finalResult.filter(function (space) {
              return space.type == 'Public' || space.type == 'Private' && space.is_member == true;
            });
          }

          _this.checkForUnread();

          _this.sortList();

          _this.loadingSubSpace = false; // this.checkForNewSubSpace();
        } else {
          axios.get('/fetch-sub-spaces-' + _this.$root.selectedSpace.general_spaceId + '-' + _this.$root.userDeviceId).then(function (response) {
            if (response.status == 200) {
              _this.$root.LocalStore('sub_channels_' + _this.$root.selectedSpace.general_spaceId + _this.$root.username, response.data);

              var _finalResult = response.data.sub_channels;

              if (_this.checkIfisOwner()) {
                _this.subSpaces = _finalResult;
              } else {
                _this.subSpaces = _finalResult.filter(function (space) {
                  return space.type == 'Public' || space.type == 'Private' && space.is_member == true;
                });
              }
    ...
    checkForUnread: function checkForUnread() {
      ...
      this.subSpaces.map(function (space) {
        var unreadStoredMsg = _this2.$root.getLocalStore('unread_messages_' + space.space_id + _this2.$root.username);
    ...
    addSubChannel: function addSubChannel() {
      ...
      setTimeout(function () {
        _this4.$root.chatComponent.innerSideBarContent = 'add_sub_channel';
      }, 500);
      this.$router.push({
                    ...
                    ? _c(
                        ...
                        [
                          _vm.that.$root.selectedSpace.type == "SubSpace"
                            ...
                            : _vm._e(),
                          _vm._v(" "),
                          _vm._l(_vm.subSpaces, function(space, index) {
                            return _c(
                              "v-card",
                              {
                                key: index,
...
var staticRenderFns = [
  function() {
    ...
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "col-8 py-0 px-0" }, [
      _c(
        "span",
