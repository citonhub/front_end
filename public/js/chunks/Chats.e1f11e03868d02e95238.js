  ...
  methods: {
    ...
    checkForUnreadMessagesDisconnected: function checkForUnreadMessagesDisconnected() {
      ...
      if (this.chatIsOpen) {
        ...
        unreadStoredMsg.then(function (result) {
          ...
          if (_this19.$root.sendingMessage == false) {
            _this19.periodicUpdate(finalResultUnread);
          }
        });
      }
    },
    checkForNewNotes: function checkForNewNotes() {
      ...
      axios.get('/fetch-diary-notes-' + this.$root.selectedSpace.bot_data.bot_id).then(function (response) {
        if (response.status == 200) {
          _this20.$root.LocalStore('diary_notes_' + _this20.$root.selectedSpace.space_id + _this20.$root.username, response.data);

          var finalResult = response.data.diary_notes;
          _this20.$root.diaryNotes = finalResult;
        }
      })["catch"](function (error) {});
    },
    fetchSpaceInfo: function fetchSpaceInfo() {
      ...
      axios.get('/fetch-space-info-' + this.$root.selectedSpace.space_id).then(function (response) {
        if (response.status == 200) {
          var storedMsg = _this21.$root.getLocalStore('full_space_' + _this21.$root.selectedSpace.space_id + _this21.$root.username);
          ...
          storedMsg.then(function (result) {
            if (result != null) {
              ...
              if (response.data.space.type == 'SubSpace') {
                ...
                _this21.$root.selectedGenSpaceType = response.data.space.gen_space.type;
                ...
                _this21.$root.selectedSubSpaceName = response.data.space.name;
                _this21.$root.selectedSubSpaceType = subSpaceData.type;
                generalSpace.sub_spaces = response.data.space.sub_spaces;
                _this21.$root.selectedSpace = generalSpace;
              } else {
                _this21.$root.selectedSpace = response.data.space;
              }

              finalResult.members = response.data.members;

              _this21.$root.LocalStore('full_space_' + _this21.$root.selectedSpace.space_id + _this21.$root.username, finalResult);
            }
          }); // update chatList

          var storedChat = _this21.$root.getLocalStore('user_chat_list' + _this21.$root.username);
            ...
            if (result != null) {
              ...
              if (response.data.space.type == 'Channel' || response.data.space.type == 'Team') {
                finalResult.channels.map(function (chat) {
                  if (chat.space_id == _this21.$root.selectedSpace.space_id) {
              ...
              var fullList = finalResult.channels.concat(finalResult.direct_messages, finalResult.pet_spaces);
              _this21.$root.ChatList = fullList;

              _this21.$root.sortChatList();

              _this21.$root.LocalStore('user_chat_list' + _this21.$root.username, finalResult);
            }
          });
        }
      })["catch"](function (error) {});
      ...
      if (this.$root.Messages.length == 0) {} else {
        ...
        storedSuggestions.then(function (result) {
          ...
          if (finalResultUnread != null) {
            _this22.$root.botSuggestionArray = finalResultUnread;
          }
        });
      }
    }
                    ...
                    _c(
                      ...
                      [
                        _vm.that.$root.showEmojiBox
                          ...
                          : _vm._e(),
                        _vm._v(" "),
                        _vm.that.$root.Messages && !this.$root.showRootReply
                          ? _c(
                              "div",
                              { staticClass: "col-6  py-1 mb-2 text-left" },
                              [
                                _vm.that.$root.Messages
                                  ? [
                                      _vm.that.$root.Messages.length > 0
                        ...
                        this.$root.selectedSpace.type != "Bot"
                          ? _c(
                              "div",
                              { staticClass: "col-6  py-1 text-right" },
                              [
                                _c(
                                  "v-btn",
                                  {
