...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    fetchProjectData: function fetchProjectData() {
      ...
      var storedProjectData = this.$root.getLocalStore('user_projects_data_' + this.$root.username);
      storedProjectData.then(function (result) {
        if (result != null) {
          var finalResult = JSON.parse(result);
          _this.$root.projectData = finalResult;
          _this.loadingProject = false; // this.checkForNewProjectData();
        } else {
          axios.get('/fetch-project-data-' + _this.$router.params.project_slug).then(function (response) {
            if (response.status == 200) {
              _this.$root.LocalStore('user_projects_data_' + _this.$root.username, response.data);
    ...
    openSettings: function openSettings() {
      this.$router.push({
        path: '/board/projects/panel/' + this.$router.params.project_slug + '/set-panel'
      });
    }
  }
});
