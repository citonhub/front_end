...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    searchChatList: function searchChatList() {
      ...
      this.$root.searchChatList = [];
      var nameValue = '';

      for (var index = 0; index < this.$root.ChatList.length; index++) {
        if (this.$root.ChatList[index].type == 'Team' || this.$root.ChatList[index].type == 'Channel') {
          nameValue = this.$root.ChatList[index].name;
        } else if (this.$root.ChatList[index].type == 'Bot' && this.$root.ChatList[index].bot_data != null) {
          nameValue = this.$root.ChatList[index].bot_data.name;
        } else if (this.$root.ChatList[index].type == 'Direct') {
          nameValue = this.$root.ChatList[index].userInfo.username;
        }

        nameValue.toLowerCase();

        if (nameValue.includes(query)) {
          this.$root.searchChatList.push(this.$root.ChatList[index]);
        }
      }

      console.log(this.$root.searchChatList);
    }
  }
});
