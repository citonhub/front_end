  ...
  methods: {
    ...
    runCodeOnSandbox: function runCodeOnSandbox() {
      ...
      axios.post('/run-code-on-sandbox', {
        ...
        if (response.status == 200) {
          ...
          if (response.data[0].status.description == 'Accepted') {
            _this2.ResultCode = response.data[0].stdout;
          } else if (response.data[0].status.description == 'In Queue') {
            _this2.ResultCode = 'In Queue...';

            _this2.checkResponse(token);
          } else if (response.data[0].status.description == 'Processing') {
            _this2.ResultCode = 'Processing...';
        ...
        _this2.ResultCode = 'An issue occured,unable to run on sandbox...' + error;
      });
    },
    ...
    filehandler: function filehandler(e) {
      ...
      if (fileSize <= 400000000) {
        fr.addEventListener('load', function () {
          _this4.fileUrl = fr.result; // send message
    ...
    sendMessageData: function sendMessageData(formData) {
      ...
      axios.post('/create-response-content', formData, {
        ...
        onUploadProgress: function onUploadProgress(progressEvent) {
          _this7.progressvalue = parseInt(Math.round(progressEvent.loaded / progressEvent.total * 100));
        }
      }).then(function (response) {
        if (response.status == 200) {
          var message = response.data.message;

          _this7.$root.noteContent.pages[_this7.selectedContentId].contents.push(message);

          _this7.clearData();

          _this7.loadingSendMsg = false;
          _this7.progressvalue = 0;
          _this7.$root.AddModalIsUp = false;
        }
      })["catch"](function (error) {
