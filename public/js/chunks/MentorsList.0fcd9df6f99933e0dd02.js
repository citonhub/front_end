    ...
    getMentors: function getMentors() {
      ...
      axios.get('/fetch-mentors' + '?page=' + this.currentPage).then(function (response) {
        if (response.status == 200) {
          _this.$root.leaderboardMembers = response.data.mentors;
          _this.mentors = _this.$root.leaderboardMembers;
          _this.loading = false;
        }
    ...
    loadNextSet: function loadNextSet(shown) {
      ...
      if (shown) {
        ...
        axios.get('/fetch-mentors' + '?page=' + this.newPage).then(function (response) {
          if (response.status == 200) {
            _this2.$root.leaderboardMembers = response.data.mentors;
            _this2.newData = _this2.$root.leaderboardMembers;

            if (_this2.$root.leaderboardMembers.length == 0) {
