    ...
    saveNote: function saveNote() {
      ...
      axios.post('/update-note-data', {
        ...
        if (response.status == 200) {
          _this.showAlert('Saved!', 'Please update diary to effect changes', 'success');
      ...
      if (type == 'question') {
        izitoast__WEBPACK_IMPORTED_MODULE_0___default.a.question({
          ...
          buttons: [['<button><b>YES</b></button>', function (instance, toast) {
            axios.post('/delete-intent-response/' + _this2.$root.pageToDelete).then(function (response) {
              if (response.status == 200) {
                var remainingPages = _this2.$root.noteContent.pages.filter(function (page) {
                  return page.slug != _this2.$root.pageToDelete;
                });

                _this2.$root.noteContent.pages = remainingPages;

                _this2.$root.LocalStore('user_diary_data_' + _this2.$route.params.diary_id + _this2.$root.username, _this2.$root.selectedDiary);

                _this2.$root.pageToDelete = '';
              }
            })["catch"](function (error) {
              _this2.$root.diaryBoardComponent.showAlert('Oops!', 'Unable to delete page', 'error');
            });
            instance.hide({
