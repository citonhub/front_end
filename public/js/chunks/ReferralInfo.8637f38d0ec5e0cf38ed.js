    ...
    fetchReferralInfo: function fetchReferralInfo() {
      ...
      storedReferral.then(function (result) {
        if (result != null) {
          ...
          _this.referralData = finalResult.referrals.data;
          _this.totalReferral = finalResult.total_referrals;
          _this.totalEarned = finalResult.amount_earned;
          _this.loading = true;

          _this.checkForReferral();
        } else {
          axios.get('/referral-info').then(function (response) {
            if (response.status == 200) {
              _this.$root.LocalStore('user_referral_info' + _this.$root.username, response.data);

              var _finalResult = response.data;
              _this.referralData = _finalResult.referrals.data;
              _this.totalReferral = _finalResult.total_referrals;
              _this.totalEarned = _finalResult.amount_earned;
              _this.loading = false;
            }
          })["catch"](function (error) {
            _this.$root.cardViewComponent.showAlert('Oops!', 'Something went wrong', 'error');
                            ...
                            _c("div", { staticClass: "ml-auto" }, [
                              _c(
                                ...
                                [
                                  ...
                                  _vm._v(
                                    _vm._s(_vm.formatMoney(user.amount_earned))
                                  )
                                ]
                              )
                            ])
