    ...
    deletePost: function deletePost() {
      ...
      axios.post('/delete-hub-post', {
        ...
        if (response.status == 200) {
          var remainingPost = [];
          _this.$root.authProfile.points -= 20;
          ...
          if (_this.fromProfile) {
            ...
            remainingPost = _this.$root.posts.filter(function (eachPost) {
              return eachPost.id != _this.post.id;
            });
            _this.$root.posts = remainingPost;

            var remainingPostSearch = _this.$root.postsSearch.filter(function (eachPost) {
              return eachPost.id != _this.post.id;
            });

            _this.$root.postsSearch = remainingPostSearch;
            _this.$root.hubComponents.loadingPost = false;
          }
        }
