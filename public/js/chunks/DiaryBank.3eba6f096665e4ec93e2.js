    ...
    checkForNewDiary: function checkForNewDiary() {
      ...
      axios.get('/get-diary-bank').then(function (response) {
        if (response.status == 200) {
          _this.$root.LocalStore('user_diary_bank_' + _this.$root.username, response.diaries);

          _this.$root.diaryBankList = response.diaries;
        }
    ...
    fetchDiaryBank: function fetchDiaryBank() {
      ...
      storedDiary.then(function (result) {
        if (result != null) {
          ...
          axios.get('/get-diary-bank').then(function (response) {
            if (response.status == 200) {
              _this2.$root.LocalStore('user_diary_bank_' + _this2.$root.username, response.diaries);

              _this2.$root.diaryBankList = response.diaries;
              _this2.loadingDiary = false;
            }
          })["catch"](function (error) {
            _this2.loadingDiary = false;
