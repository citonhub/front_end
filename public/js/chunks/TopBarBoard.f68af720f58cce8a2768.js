  ...
  methods: {
    ...
    triggerSearch: function triggerSearch() {
      ...
      if (this.$root.searchType == 'leaderboard') {
        thsearchLeaderboard(this.searchContent);
      }
    },
    ...
    searchLeaderboard: function searchLeaderboard(query) {
      ...
      if (this.$router.currentRoute.path.indexOf('leaderboard') >= 0) {
        ...
        //   })
        this.loading = true;
        axios.get('/fetch-mentors' + '?page=' + this.currentPage + '&query' + query).then(function (response) {
          if (response.status == 200) {
            _this3.$root.leaderboardMembers = response.data.mentors;
            _this3.mentors = _this3.$root.leaderboardMembers;
            _this3.loading = false;
          }
        })["catch"](function (error) {
          _this3.loading = false;

          _this3.showAlert('Oops!', 'Unable to fetch mentors,please try again', 'error');
        });
      }
    },
