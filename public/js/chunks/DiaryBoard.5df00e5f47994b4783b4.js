      ...
      if (type == 'question') {
        izitoast__WEBPACK_IMPORTED_MODULE_0___default.a.question({
          ...
          buttons: [['<button><b>YES</b></button>', function (instance, toast) {
            axios.post('/delete-intent-response/' + _this3.$root.pageToDelete).then(function (response) {
              if (response.status == 200) {
                var remainingPages = _this3.$root.noteContent.pages.filter(function (page) {
                  return page.slug != _this3.$root.pageToDelete;
                });

                _this3.$root.noteContent.pages = remainingPages;

                _this3.$root.LocalStore('user_diary_data_' + _this3.$route.params.diary_id + _this3.$root.username, _this3.$root.selectedDiary);

                _this3.$root.pageToDelete = '';
              }
            })["catch"](function (error) {
              _this3.$root.diaryBoardComponent.showAlert('Oops!', 'Unable to delete page', 'error');
            });
            instance.hide({
