  ...
  methods: {
    TopIsVisible: function TopIsVisible(isVisible, entry) {
      if (isVisible) {
        ...
        if (this.counter > 0) {
          var NextArrays = this.$root.returnedMessages.slice(this.$root.messageSlicePosition, this.$root.returnedMessages.length);

          for (var index = 0; index < NextArrays.length; index++) {
            this.$root.Messages.unshift(NextArrays[index]);
          }
        }
