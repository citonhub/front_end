  ...
  methods: {
    selectDiaryNote: function selectDiaryNote(note) {
      if (this.$root.channelBottomComp) {
        ...
        this.clearTracker(note, this.diaryNotes);
      }
    ...
    clearTracker: function clearTracker(note, fullNote) {
      var _this2 = this;
      ...
      axios.post('/clear-update-track', {
        ...
        if (response.status == 200) {
          fullNote.map(function (eachnote) {
          ...
          _this2.$root.LocalStore('diary_notes_' + _this2.$root.selectedSpace.space_id + _this2.$root.username, fullNote);
        }
      })["catch"](function (error) {});
    },
