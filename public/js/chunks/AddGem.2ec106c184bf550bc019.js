...
var VideoPlayer = function VideoPlayer() {
  return Promise.all(/*! import() | VideoPlayer?v=0.13 */[__webpack_require__.e("vendors~ProfilePage~VideoPlayer"), __webpack_require__.e("VideoPlayer")]).then(__webpack_require__.bind(null, /*! ../chats/VideoPlayer.vue */ "./resources/js/components/chats/VideoPlayer.vue"));
};
...
/* harmony default export */ __webpack_exports__["default"] = ({
  data: function data() {
    ...
    return _ref = {
      ...
      "id": 46,
      "name": 'TYPESCRIPT'
    }]), _defineProperty(_ref, "CodeFilename", 'index'), _defineProperty(_ref, "loadingAddPage", false), _defineProperty(_ref, "timer", '0:00'), _defineProperty(_ref, "seconds", 0), _defineProperty(_ref, "minute", 0), _defineProperty(_ref, "recorderInterval", null), _defineProperty(_ref, "recording", false), _defineProperty(_ref, "audioBlob", ''), _defineProperty(_ref, "mediaRecorder", null), _ref;
  },
  components: {
  ...
  methods: {
    ...
    startrecord: function startrecord() {
      ...
      navigator.mediaDevices.getUserMedia({
        ...
        _this2.mediaRecorder.addEventListener("stop", function () {
          _this2.audioBlob = new Blob(_this2.audioChunks);
          _this2.$root.recorderBlob = new Blob(_this2.audioChunks);
          _this2.$root.recordUrl = URL.createObjectURL(_this2.audioBlob);
        });
      });
    },
    startCounter: function startCounter() {
      ...
      this.recorderInterval = setInterval(function () {
        ...
        if (_this3.seconds < 10) {
          secondsString = '0' + _this3.seconds;
        } else {
          if (_this3.seconds == 60) {
            ...
            secondsString = _this3.seconds;
          }
        }

        _this3.timer = _this3.minute + ':' + secondsString;
      }, 1000);
    },
    ...
    saveContentOrder: function saveContentOrder() {
      ...
      axios.post('/save-content-order', {
        ...
        if (response.status == 200) {
          if (showAlert) {
            _this4.$root.diaryBoardComponent.showAlert('Saved!', 'Your changes have been saved', 'success');
          }

          _this4.$root.LocalStore('user_diary_data_' + _this4.$route.params.diary_id + _this4.$root.username, _this4.$root.selectedDiary);
        }
      })["catch"](function (error) {
        _this4.$root.diaryBoardComponent.showAlert('Oops!', 'Unable to save changes,please try again', 'error');
      });
    },
    ...
    runCodeOnSandbox: function runCodeOnSandbox() {
      ...
      axios.post('/run-code-on-sandbox', {
        ...
        if (response.status == 200) {
          ...
          if (response.data[0].status.description == 'Accepted') {
            _this5.ResultCode = response.data[0].stdout;
          } else if (response.data[0].status.description == 'In Queue') {
            _this5.ResultCode = 'In Queue...';

            _this5.checkResponse(token);
          } else if (response.data[0].status.description == 'Processing') {
            _this5.ResultCode = 'Processing...';
        ...
        _this5.ResultCode = 'An issue occured,unable to run on sandbox...' + error;
      });
    },
    addNewContent: function addNewContent() {
      ...
      axios.get('/add-new-page/' + this.$root.noteContent.note.tag_unique_id + '/' + this.$route.params.diary_id).then(function (response) {
        if (response.status == 200) {
          _this6.$root.noteContent.pages.push(response.data);

          _this6.loadingAddPage = false;
        }
      })["catch"](function (error) {
        _this6.$root.diaryBoardComponent.showAlert('Oops!', 'Unable to add page,please try again', 'error');
    ...
    filehandler: function filehandler(e) {
      ...
      if (fileSize <= 400000000) {
        fr.addEventListener('load', function () {
          _this8.fileUrl = fr.result; // send message
    ...
    vidoehandler: function vidoehandler(e) {
      ...
      if (fileSize <= 100000000) {
        fr.addEventListener('load', function () {
          _this10.videoUrl = fr.result; // send message
    ...
    sendMessageData: function sendMessageData(formData) {
      ...
      axios.post('/create-response-content', formData, {
        ...
        onUploadProgress: function onUploadProgress(progressEvent) {
          _this11.progressvalue = parseInt(Math.round(progressEvent.loaded / progressEvent.total * 100));
        }
      }).then(function (response) {
        if (response.status == 200) {
          var message = response.data.message;

          _this11.$root.noteContent.pages[_this11.selectedContentId].contents.push(message);

          _this11.saveContentOrder(false);

          _this11.clearData();

          _this11.loadingSendMsg = false;
          _this11.progressvalue = 0;
          _this11.$root.AddModalIsUp = false;
        }
      })["catch"](function (error) {
        _this11.showAlert('Oops!', 'Something went wrong, please try again', 'error');
