    ...
    AddItemsToResources: function AddItemsToResources() {
      ...
      axios.post('/add-items-to-resource', {
        ...
        if (response.status == 200) {
          _this.loading = false;
          console.log(_this.$root.selectedResource);

          if (_this.$root.selectedResource.type == 'playlist') {
            _this.$root.chatComponent.showAlert('Added!', 'video added to playlist', 'success');
          } else {
            _this.$root.chatComponent.showAlert('Added!', 'the resource has been added', 'success');
          }
