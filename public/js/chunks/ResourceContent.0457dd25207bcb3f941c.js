  ...
  methods: {
    ...
    checkForNewResourceContent: function checkForNewResourceContent() {
      ...
      axios.get('/fetch-resource-content/' + this.$root.selectedResource.resource_id).then(function (response) {
        if (response.status == 200) {
          ...
          if (_this4.$root.resourceComponent) {
            _this4.$root.resourceComponent.saveResourceContentOrder(false);
          }
        }
      })["catch"](function (error) {
    ...
    nextPagehandler: function nextPagehandler(shown) {
      ...
      if (this.nextPageToken && shown) {
        ...
        axios.get('/fetch-resource-content/' + this.$root.selectedResource.resource_id + '/' + playlistID + '/' + this.nextPageToken).then(function (response) {
          if (response.status == 200) {
            var results = [];
