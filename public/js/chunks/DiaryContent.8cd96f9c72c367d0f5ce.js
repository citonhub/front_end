...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    addNote: function addNote() {
      ...
      axios.get('/add-new-note/' + this.$route.params.diary_id).then(function (response) {
        if (response.status == 200) {
          ...
          _this.$router.push({
            path: '/board/diary/board/' + _this.$route.params.diary_id + '/add-note/' + _this.$root.noteContent.tag_unique_id
          });
        }
    ...
    showNote: function showNote(note) {
      ...
      this.$router.push({
        path: '/board/diary/board/' + this.$route.params.diary_id + '/add-note' + this.$root.noteContent.tag_unique_id
      });
    },
